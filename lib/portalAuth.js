...
export async function resolveSession(req) {
  ...
  // Sliding refresh: bump expires_at to now + 90 days on every valid lookup so
  // active users never get kicked out. Fire-and-forget; failure to extend just
  // means this session lives out its current TTL, not a request-blocking error.
  const newExpiry = new Date(Date.now() + SLIDING_TTL_MS).toISOString();
  if (new Date(session.expires_at).getTime() < Date.now() + SLIDING_TTL_MS) {
    supabaseAdmin.from('sessions').update({ expires_at: newExpiry }).eq('token', token)
      .then(() => {}, () => {});
  }
...
// 90-day sliding window. Each authed API call extends the session to (now + 90d)
// so daily users never expire. Cap on absolute lifetime is enforced at login
// (api/auth.js sets the initial TTL based on the Remember Me checkbox).
