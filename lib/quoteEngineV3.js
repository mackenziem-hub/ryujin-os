...
const DEFAULTS = {
  ...
  priceRounding: 25,
  laborRoofing: {
    // Asphalt rates aligned with subcontractor-rates.js Ryan 2025 actualized sheet
    // (Apr 28 2026). Only used as fallback when computeSubPaysheet throws — sub
    // paysheet path is the truth for asphalt offers.
    asphalt: { low: 110, moderate: 135, steep: 160 },
    ...
    pipe_flashing: 20,
    small_chimney_flashing: 150,
    large_chimney_flashing: 200,
...
async function loadTenantSettings(supabase, tenantId) {
  ...
  if (!data) {
    return {
      ...
      mobilization: null,
      remediation: null,
      // Supervisor + small-job + sub defaults
      supervisorDayRate: 270,
      supervisorRequired: true,
      defaultSubSlug: 'atlantic-roofing',
      smallJobThresholdSq: 15,
      smallJobSurchargeAmount: 500,
      // Custom multiplier breakeven guard (Phase 4)
      minCustomMultiplier: 1.35,
      // Real-cash net reporting (Phase 6)
      realLoadingPct: 0.12,
      // Bundle waste padding toggle (Phase 5)
      applyWasteToBundles: false
    };
  }
  ...
  return {
    ...
    // Supervisor (migration 022). parseFloat handles numeric→string roundtrip.
    ...
    // Production assistant (migration 100). Optional extra crew labor at a day
    // rate; priced only when measurements.productionAssistantDays > 0.
    productionAssistantDayRate: data.production_assistant_day_rate != null ? parseFloat(data.production_assistant_day_rate) : 115,
    ...
    // Custom multiplier breakeven guard (migration 024, Phase 4). 1.35 = 0% profit on 35% S+M+O.
    minCustomMultiplier: data.min_custom_multiplier != null ? parseFloat(data.min_custom_multiplier) : 1.35,
    // Real-cash net reporting (migration 024, Phase 6). Lean overhead allocation for Plus Ultra.
    realLoadingPct: data.real_loading_pct != null ? parseFloat(data.real_loading_pct) : 0.12,
    // Bundle waste padding toggle (migration 024, Phase 5). False = order at 3 bundles/SQ flat.
    applyWasteToBundles: data.apply_waste_to_bundles === true,
    branding: {
...
function calculateQuantities(itemKey, measurements, config = {}) {
  ...
  // Bundle items (shingles) — Mac's real ordering is 3/SQ flat, no waste pad.
  // Setting controlled per tenant. When false, use measuredSQ instead of totalSQ.
  const shingleSQ = applyWasteToBundles ? totalSQ : measuredSQ;

  switch (itemKey) {
    // ── Roofing Materials ──
    case 'shingles': {
      const bps = config.bundles_per_sq || 3;
      return { qty: Math.ceil(shingleSQ * bps), unit: 'bundle' };
    }
    case 'underlayment':
...
function resolveLaborOrCalculatedCost(key, pitchTier, distanceKM, config, measurements, settings) {
  ...
  switch (key) {
    // ── Roofing ──
    case 'base_labor': {
      if (config.system === 'metal') return (r.metal && r.metal[pitchTier]) || 290;
      if (config.system === 'flat') return (r.flat && r.flat[pitchTier]) || 100;
      return (r.asphalt && r.asphalt[pitchTier]) || 130;
    }
    case 'tearoff_labor': {
      // Asphalt tear-off is BUNDLED into base_labor ($130/$160/$190 per SQ) per
      // pricing_formula_v2.md Section 5 — return 0 to avoid double-counting.
      // Metal tear-off is a SEPARATE line at $40/SQ per Metal SOP Section 8.
      if (config.system === 'metal') return 40;
      return 0;
    }
    case 'extra_layer_labor':
    ...
    case 'chimney_labor':
      return (config.chimneySize === 'large') ? (r.large_chimney_flashing || 200) : (r.small_chimney_flashing || 150);
    case 'cricket_labor':
...
export async function calculateQuoteV3(supabase, {
  ...
  // 2. Calculate roof metrics — single-pitch OR multi-plane
  //
  // Multi-plane mode (May 7 2026): measurements.planes = [{sqft, pitch, label?}, ...]
  // When provided, each plane gets its own pitch multiplier for area uplift AND
  // its own pitch band for sub-paysheet labor. Solves the Kataria-class problem
  // where a 5/12 main + 12/12 rakes single-pitch input under-priced steep sections.
  // Materials still pick rates from the dominant pitch (largest plane) — material
  // SKUs don't band by pitch the way labor does.
  let pitch = measurements.pitch || '5/12';
  let sqft = measurements.squareFeet || 0;
  ...
  let adjustedSqFt;
  let measuredSQ;
  let planeBreakdown = null;

  const inputPlanes = Array.isArray(measurements.planes) ? measurements.planes.filter(p => p && Number(p.sqft) > 0 && p.pitch) : [];

  if (inputPlanes.length > 0) {
    let totalAdjusted = 0;
    let totalRaw = 0;
    planeBreakdown = inputPlanes.map(p => {
      const pm = PITCH_MULTIPLIERS[p.pitch] || 1.083;
      const raw = Number(p.sqft) || 0;
      const adjusted = raw * pm;
      ...
      return {
        ...
        pitch: p.pitch,
        pitchMult: pm,
        adjustedSqft: Math.round(adjusted),
        sq: Math.ceil(adjusted / 100)
      };
    });
    // Dominant pitch (largest plane by 2D sqft) — picks material rates only.
    const dom = [...planeBreakdown].sort((a, b) => b.sqft - a.sqft)[0];
    pitch = dom.pitch;
    pitchMult = dom.pitchMult;
    adjustedSqFt = totalAdjusted;
    measuredSQ = Math.ceil(totalAdjusted / 100);
    sqft = totalRaw; // overwrite so downstream consumers see the real total
  } else {
    pitchMult = PITCH_MULTIPLIERS[pitch] || 1.083;
    // areaIsPitchAdjusted: caller passed an ALREADY-pitched roof area (e.g. an
    // EagleView "Total Roof Area", which is the sloped surface, not the 2D
    // footprint). Use it as the actual roof area directly — do NOT apply the
    // pitch multiplier a second time, or the job is over-measured by the pitch
    // factor (8% at 5/12 up to 41% at 12/12). The pitch value is still used for
    // labor banding via getLaborPitchTier(pitch) below, so steep-slope labor is
    // preserved. Default (flag absent/false) keeps the footprint contract: the
    // engine pitch-adjusts raw 2D sqft itself.
    if (measurements.areaIsPitchAdjusted) {
      adjustedSqFt = sqft;
      measuredSQ = sqft > 0 ? Math.ceil(sqft / 100) : 0;
    } else {
      adjustedSqFt = sqft * pitchMult;
      measuredSQ = sqft > 0 ? Math.ceil(adjustedSqFt / 100) : 0;
    }
  }
  ...
  const fullMeasurements = {
    ...
    // Phase 5 (Apr 27): bundle waste toggle. Plus Ultra default false — Mac
    // orders bundles flat at 3/SQ, no waste pad. EOS does it the same way.
    // Labor still uses measuredSQ pre-waste per SOP Section 5.
    applyWasteToBundles: settings.applyWasteToBundles === true
  };
  ...
  // ── Sub paysheet path ──
  // Asphalt offers route their labor cost through computeSubPaysheet (real
  // Ryan paysheet) so the engine's hard cost stays in lockstep with the actual
  // sub bill — travel, waste, ridge vent LF, valley metal LF, mansard, etc.
  // Metal does NOT use this path (Ryan's sheet is asphalt-only; metal labor
  // resolves to $250/$290/$350 per pitch via the engine's own resolver).
  // Exterior + combined offers also fall back to the engine's tenant rates.
  ...
  // Combined offers (gold-shell, platinum-shell) include an asphalt roof and
  // need Ryan's full paysheet just like pure asphalt. Without this, combined
  // jobs silently fall back to engine flat-rate labor + miss travel/waste/
  // bundle install costs.
  ...
  for (const tmpl of scopeTemplate) {
    ...
    const config = {
      ...
      system: (tmpl.config && tmpl.config.system) || offerSystem
    };
    ...
    if (useSubPaysheet && (SUB_LABOR_KEYS.has(key) || key === 'disposal')) {
      const isDisposal = key === 'disposal';
      lineItems.push({
        item_key: key, category: tmpl.category,
        // The $0 here is correct (the real cost is in the sub paysheet), but a bare
        // "Disposal $0" reads as free. Label it so it reads as bundled, not skipped.
        label: isDisposal ? `${tmpl.label} (included in crew waste removal)` : tmpl.label,
        config, quantity: qty, unit, unit_cost: 0, total_cost: 0,
        price_source: 'calculated',
        source_product_id: null,
        source_detail: isDisposal
          ? 'Disposal is carried in the crew waste-removal line of the sub paysheet, not billed separately. The $0 here avoids double-counting.'
          : 'Included in subcontractor labor (see paysheet breakdown below)',
        is_override: false,
        original_cost: null,
        included: false,
        estimated: false,
  ...
  if (useSubPaysheet && totalSQ > 0) {
    ...
    try {
      ...
      // planes (May 7 2026): if multi-plane input was provided, pass per-plane SQ
      // through so base labor splits at each plane's pitch band ($110/$135/$160/$180).
      const planesForSub = planeBreakdown
        ? planeBreakdown.map(p => ({ sq: p.sq, pitch: p.pitch, label: p.label }))
        : null;
      paysheet = computeSubPaysheet({
        ...
        // Big-job waste removal override (e.g. duplex teardowns needing multiple loads).
        // Falls back to flat band rate when 0/unset.
        waste_removal_override: measurements.wasteRemovalOverride || measurements.waste_removal_override || 0
      }, tier, scopeExtras, subSlug);
    } catch (e) {
      // Fall back so the engine ships a quote rather than crashing on a missing
      // rate sheet. Hard cost will be light from engine fallback labor — log
      // loudly so the gap is visible in Vercel function logs.
      console.warn(`[quoteEngineV3] computeSubPaysheet threw for offer ${offer.slug} — falling back to engine labor. ${e?.message || e}`);
      paysheet = null;
    }
  ...
  // ── Production assistant (optional extra crew labor) ──
  // Some jobs run a production assistant alongside the sub crew. Off by default
  // (0 days = no line, no cost change); set measurements.productionAssistantDays
  // to price it in. Rate from tenant_settings (migration 100, default $115/day).
  const paDays = Math.max(0, parseInt(measurements.productionAssistantDays ?? measurements.production_assistant_days ?? 0, 10) || 0);
  if (useSubPaysheet && paDays > 0 && settings.productionAssistantDayRate > 0) {
    const paFee = paDays * settings.productionAssistantDayRate;
    lineItems.push({
      item_key: 'production_assistant_fee', category: 'labor',
      label: `Production assistant (${paDays} day${paDays > 1 ? 's' : ''} @ $${settings.productionAssistantDayRate})`,
      config: {}, quantity: paDays, unit: 'day',
      unit_cost: settings.productionAssistantDayRate, total_cost: paFee,
      price_source: 'calculated',
      source_detail: 'Production assistant day rate from tenant_settings',
      included: true, sort_order: 945
    });
    hardCost += paFee;
  }
  ...
  // 5. Pricing: two methods supported
  //
  // ASPHALT (multiplier method) — Plus Ultra/Sales/pricing_formula_v2.md
  //   Multiplier = 1 + 0.10 (commission) + 0.05 (marketing) + 0.20 (overhead) + target_profit
  //   Gold 1.47 (12%), Platinum 1.52 (17%), Diamond 1.58 (23%). 35% S+M+O baked in.
  //
  // METAL (divisor / additive cost stack) — Plus Ultra/Sales/Metal_Roofing_Pricing_Logic_SOP.pdf
  //   Sell = Direct Cost ÷ (1 − 0.10 − 0.05 − 0.20 − net_target)
  //   Standard 0.53 (12%), Enhanced 0.50 (15%), Premium 0.48 (17%).
  //   Asphalt multipliers compress metal margins to zero — never use 1.47/1.52/1.58 on metal.
  const pricingMethod = String(offer.pricing_method || 'multiplier').toLowerCase();
  ...
  if (pricingMethod === 'divisor') {
    // Divisor stored in multipliers.divisor; fall back to .local for backward compat.
    divisor = Number(multipliers.divisor || multipliers.local) || 0.53;
    sellingPrice = roundToNearest(hardCost / divisor, settings.priceRounding);
  } else {
    multiplier = (allowProjectTypeBump ? multipliers[projectType] : multipliers.local) || multipliers.local || 1.47;
    // Instant Estimator runs commission-free (no sales rep on self-serve leads) to stay
    // competitive. Strip the 0.10 commission component baked into the multiplier. Opt-in
    // only: proposals, contracts and admin estimates pass null and are byte-identical.
    if (commissionPct === 0) {
      multiplier = +(multiplier - 0.10).toFixed(4);
    }
    sellingPrice = roundToNearest(hardCost * multiplier, settings.priceRounding);
  }

  // Custom multiplier breakeven guard. Only meaningful for multiplier offers —
  // 1.35 = exactly 0% profit on 35% S+M+O loading. Divisor offers have their
  // own structural floor (divisor < 1.0 always positive net by construction).
  const minCustomMultiplier = settings.minCustomMultiplier != null ? settings.minCustomMultiplier : 1.35;
  const belowBreakeven = pricingMethod === 'multiplier' && multiplier < minCustomMultiplier;
  const breakevenWarning = belowBreakeven
    ? `Custom multiplier ${multiplier.toFixed(2)} is below the ${minCustomMultiplier.toFixed(2)} minimum required to cover overhead. This quote is structurally a loss.`
    : null;
  ...
  // residential reroofs. Mac confirmed Apr 27.
  ...
  // grossMargin = (sell − hardCost) / sell. Pre-S+M+O. Kept under legacy
  // name `netMargin` for backward compat with consumers (proposal-client etc).
  const netMargin = sellingPrice > 0
    ? Math.round(((sellingPrice - hardCost) / sellingPrice) * 1000) / 10
    : 0;

  // ── SOP profit (canonical) ──
  // Asphalt: target profit is % OF HARD COST per pricing_formula_v2.md Section 3.
  //   Gold 12% / Platinum 17% / Diamond 23%. Multiplier bakes in the 35% S+M+O.
  // Metal: target profit is % OF SELLING PRICE per Metal_Roofing_Pricing_Logic_SOP.pdf.
  //   Standard 12% / Enhanced 15% / Premium 17%. Divisor = 1 − 0.35 − target.
  // Both interpretations represent "net to Mac after S+M+O allocations are paid."
  const SOP_TARGETS = {
    gold: 0.12, platinum: 0.17, diamond: 0.23,
    'metal-standard': 0.12, 'metal-enhanced': 0.15, 'metal-premium': 0.17,
    'metal-americana': 0.12, 'metal-standing-seam': 0.15
  };
  const slug = String(offer.slug || '').toLowerCase();
  let sopTargetPct;
  if (SOP_TARGETS[slug] != null) {
    sopTargetPct = SOP_TARGETS[slug];
  } else if (pricingMethod === 'divisor') {
    sopTargetPct = Math.max(0, +(0.65 - divisor).toFixed(4));
  } else {
    sopTargetPct = Math.max(0, +(multiplier - 1.35).toFixed(4));
  }
  const sopProfit = pricingMethod === 'divisor'
    ? Math.round(sellingPrice * sopTargetPct * 100) / 100
    : Math.round(hardCost * sopTargetPct * 100) / 100;
  const sopNet = sopProfit; // identity: SOP says "net = profit once S+M+O paid"
  const sopNetPerWorkday = effectiveWorkdays > 0
    ? Math.round((sopNet / effectiveWorkdays) * 100) / 100
    : sopNet;

  // ── Real-cash net (lean reality) ──
  // Per project_plus_ultra_economics.md — Mac's actual overhead is much leaner
  // than the SOP-allocated 35%. Configurable per-tenant. Default 0.12 for Plus Ultra.
  // realCashNet = sell − hardCost − (sell × real_loading_pct).
  const realLoadingPct = settings.realLoadingPct != null ? settings.realLoadingPct : 0.12;
  const realLoadingAmount = Math.round(sellingPrice * realLoadingPct * 100) / 100;
  const realCashNet = Math.round((sellingPrice - hardCost - realLoadingAmount) * 100) / 100;
  const realCashNetPerWorkday = effectiveWorkdays > 0
    ? Math.round((realCashNet / effectiveWorkdays) * 100) / 100
    : realCashNet;
  const realCashNetMargin = sellingPrice > 0
    ? Math.round((realCashNet / sellingPrice) * 1000) / 10
    : 0;
  ...
  return {
    ...
    measurements: {
      ...
      valleysLF: fullMeasurements.valleysLF,
      // Multi-plane breakdown (May 7 2026). Null when single-pitch input used.
      planes: planeBreakdown,
      multiPitch: planeBreakdown !== null,
    ...
    summary: {
      ...
      pricingMethod,
      multiplier: pricingMethod === 'multiplier' ? multiplier : null,
      divisor: pricingMethod === 'divisor' ? divisor : null,
      sellingPrice,
      tax: hst,
      ...
      netMargin: `${netMargin}%`, // gross margin under legacy name (kept for backward compat)
      hasEstimatedPricing,
      ...
      // ── SOP profit (canonical, per pricing_formula_v2.md Section 3) ──
      // Theoretical profit-to-Mac after embedded 35% S+M+O is paid. Use for sales-price planning.
      sopTargetPct,
      sopProfit,
      sopNet,
      sopNetPerWorkday,
      // ── Real-cash net (lean reality, per project_plus_ultra_economics.md) ──
      // Real overhead allocation (default 12%) — use for "am I making money on this job".
      realLoadingPct,
      realLoadingAmount,
      realCashNet,
      realCashNetPerWorkday,
      realCashNetMargin: `${realCashNetMargin}%`,
      // ── Custom multiplier breakeven guard (Phase 4, Apr 27) ──
      belowBreakeven,
      breakevenWarning,
      minCustomMultiplier,
      // ── Small-job mobilization surcharge ──
...
export async function calculateMultiOfferQuote(supabase, {
  tenantId, offerIds, measurements, overrides = {}, choices = {}, extras = [], commissionPct = null
}) {
