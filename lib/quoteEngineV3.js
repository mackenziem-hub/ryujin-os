...
export async function calculateQuoteV3(supabase, {
  ...
  //
  // CANONICAL: Plus Ultra/Sales/pricing_formula_v2.md
  //   Multiplier = 1 + 0.10 (commission) + 0.05 (marketing) + 0.20 (overhead) + target_profit
  //   Gold 1.47 (12% target), Platinum 1.52 (17%), Diamond 1.58 (23%).
  //   The 35% S+M+O is ALREADY IN the multiplier — never subtract another loading layer.
  //   Selling Price = Hard Cost × Multiplier. No bumping, no second loading.
  //
  // Distance is a LABOR adder (Section 5), NOT a multiplier bump.
  // Only use projectType-specific multipliers for truly-remote "extendedStay" jobs
  // where the offer explicitly opts in via offer.use_project_type_multipliers.
  ...
  // Custom multiplier breakeven guard. Multiplier of 1.35 = exactly 0% profit on
  // the SOP's 35% S+M+O loading. Anything below is a structural loss. Flag-only.
  const minCustomMultiplier = settings.minCustomMultiplier != null ? settings.minCustomMultiplier : 1.35;
  const belowBreakeven = multiplier < minCustomMultiplier;
  ...
  // Per pricing_formula_v2.md Section 3, target profit is:
  //   gold 12%, platinum 17%, diamond 23% — all OF HARD COST.
  // Per Section 16 (Mountain Rd worked example): profit at multiplier IS net to
  // Mac AFTER the embedded 35% S+M+O is paid out. We compute it directly from
  // hard cost × target_pct so it stays canonical even when scope changes the
  // multiplier (e.g. economy at 1.40 for asphalt-flat).
  // Map slug → SOP target. Anything outside the canonical set defaults to (multiplier − 1.35).
  const SOP_TARGETS = { gold: 0.12, platinum: 0.17, diamond: 0.23 };
  const slug = String(offer.slug || '').toLowerCase();
  const sopTargetPct = SOP_TARGETS[slug] != null
    ? SOP_TARGETS[slug]
    : Math.max(0, +(multiplier - 1.35).toFixed(4));
  const sopProfit = Math.round(hardCost * sopTargetPct * 100) / 100;
  const sopNet = sopProfit; // identity: SOP says "net = profit at multiplier" once S+M+O paid
