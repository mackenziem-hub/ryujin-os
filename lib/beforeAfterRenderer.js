...
function buildOverlay({ width, height, address, product, brandColor }) {
  ...
  const beforePath = pathFor(ttsBold, 'BEFORE', { x: 130, y: badgeY + badgeH / 2 + 10, fontSize: badgeFontSize, anchor: 'center baseline', letterSpacing: 3 });
  const afterPath = pathFor(ttsBold, 'AFTER', { x: halfW + 120, y: badgeY + badgeH / 2 + 10, fontSize: badgeFontSize, anchor: 'center baseline', letterSpacing: 3 });
  const plusUltraPath = pathFor(ttsBold, 'PLUS ULTRA', { x: width - 120, y: badgeY + 22, fontSize: 16, anchor: 'center baseline', letterSpacing: 2 });
  const roofingPath = pathFor(ttsBold, 'ROOFING', { x: width - 120, y: badgeY + 38, fontSize: 11, anchor: 'center baseline', letterSpacing: 2 });
