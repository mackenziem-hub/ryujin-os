...
// Embed Roboto Medium + ExtraBold directly into the SVG as base64-encoded
// @font-face data URLs. Vercel's serverless runtime ships no usable
// sans-serif font for librsvg, so `font-family="Arial"` falls back to
// tofu rectangles ("missing glyph" boxes). Reading the TTFs once at
// module init keeps per-render cost at zero.
const __dirname = dirname(fileURLToPath(import.meta.url));
const FONT_DIR = join(__dirname, 'assets', 'fonts');
const FONT_MEDIUM_B64 = readFileSync(join(FONT_DIR, 'Roboto-Medium.ttf')).toString('base64');
const FONT_BOLD_B64 = readFileSync(join(FONT_DIR, 'Roboto-ExtraBold.ttf')).toString('base64');
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  <defs>
    <style type="text/css"><![CDATA[
      @font-face { font-family: 'RJSans'; font-weight: 500; src: url('data:font/ttf;base64,${FONT_MEDIUM_B64}') format('truetype'); }
      @font-face { font-family: 'RJSans'; font-weight: 800; src: url('data:font/ttf;base64,${FONT_BOLD_B64}') format('truetype'); }
    ]]></style>
  ...
  <!-- BEFORE label (no backplate; black stroke keeps it legible on any background) -->
  <text x="130" y="${badgeY + badgeH / 2 + 10}" font-family="RJSans" font-size="${badgeFontSize}" font-weight="800" letter-spacing="3" fill="${brandColor}" stroke="#000" stroke-width="3" paint-order="stroke" text-anchor="middle">BEFORE</text>

  <!-- AFTER label -->
  <text x="${halfW + 120}" y="${badgeY + badgeH / 2 + 10}" font-family="RJSans" font-size="${badgeFontSize}" font-weight="800" letter-spacing="3" fill="#4ade80" stroke="#000" stroke-width="3" paint-order="stroke" text-anchor="middle">AFTER</text>

  <!-- Bottom strip -->
  <rect x="0" y="${stripY}" width="${width}" height="${stripH}" fill="url(#strip)"/>
  <text x="40" y="${stripY + stripH * 0.55}" font-family="RJSans" font-size="${addrFont}" font-weight="800" fill="#ffffff">${escapeXml(addr)}</text>
  ${prod ? `<text x="40" y="${stripY + stripH * 0.55 + addrFont + 6}" font-family="RJSans" font-size="${prodFont}" font-weight="500" fill="#e0e6f0">${escapeXml(prod)}</text>` : ''}

  <!-- PLUS ULTRA wordmark top-right (no backplate) -->
  <text x="${width - 120}" y="${badgeY + 22}" font-family="RJSans" font-size="16" font-weight="800" letter-spacing="3" fill="#ffffff" stroke="#000" stroke-width="2.5" paint-order="stroke" text-anchor="middle">PLUS ULTRA</text>
  <text x="${width - 120}" y="${badgeY + 38}" font-family="RJSans" font-size="11" font-weight="700" letter-spacing="3" fill="${brandColor}" stroke="#000" stroke-width="2" paint-order="stroke" text-anchor="middle">ROOFING</text>
</svg>`;
