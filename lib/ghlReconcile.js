...
// Cross-check won GHL opps vs Supabase accepted estimates. Pure. Returns findings[].
export function crossCheckGhl(wonOpps, estimates, customers) {
  const custById = new Map((customers || []).map((c) => [c.id, c]));
  const bySurname = new Map();
  for (const e of estimates || []) {
    if (String(e.status) !== 'accepted') continue;
    const nm = custById.get(e.customer_id)?.full_name || '';
    const s = surname(nm);
    if (s) bySurname.set(s, { final: Number(e.final_accepted_total) || 0, estimate_number: e.estimate_number, name: nm, id: e.id });
  }
  const findings = [];
  for (const o of wonOpps || []) {
    ...
    if (!m) {
      findings.push({
        ...
        detail: `Won in GHL ($${o.value.toLocaleString()}) but no matching accepted estimate in Estimator OS.`,
    ...
    } else if (m.final > 0 && Math.abs(o.value - m.final) / m.final > MISMATCH_TOLERANCE) {
      findings.push({
        kind: 'GHL_VALUE_MISMATCH', job: m.name.slice(0, 40),
        detail: `GHL won value $${o.value.toLocaleString()} vs estimate #${m.estimate_number} $${m.final.toLocaleString()} differ by more than 20% (beyond an HST-basis gap).`,
        proposed_fix: 'Reconcile which figure is correct — GHL is often pre-tax or stale; the payment receipt is authoritative.',
        dollar_impact: Math.round(Math.abs(o.value - m.final)), estimate_id: m.id,
      });
    }
  }
  return findings;
