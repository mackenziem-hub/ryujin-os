...
// percentThreshold: 25 | 50 | 75 | 95 (95 ≈ ThruPlay for longer videos)
// Uses Meta's TEMPLATE-based rule format (the explicit-filter format returns
// "Rule Format Not Available" subcode 1870049 on newer accounts).
export async function createVideoViewAudience({ videoId, name, percentThreshold, retentionDays = 30 }) {
  ...
  const TEMPLATE_BY_PCT = {
    25: 'VIDEO_VIEWERS_25_PERCENT',
    50: 'VIDEO_VIEWERS_50_PERCENT',
    75: 'VIDEO_VIEWERS_75_PERCENT',
    95: 'VIDEO_VIEWERS_95_PERCENT'
  };
  const template = TEMPLATE_BY_PCT[percentThreshold];
  if (!template) throw new Error(`Unsupported percent threshold: ${percentThreshold}`);
  ...
  const resp = await fetch(`${GRAPH_API}/${adAccountId}/customaudiences`, {
    ...
    body: JSON.stringify({
      ...
      rule,
      description: `Auto-created by Shenron — viewers who hit ${percentThreshold}% of video ${videoId}, last ${retentionDays} days`
    })
  });
  if (!resp.ok) {
    const body = await resp.text().catch(() => '');
...
export async function sendCAPIEvent({ eventName, eventTime, eventId, sourceUrl, userData, customData }) {
  ...
  const SKIP_HASH = new Set(['ip', 'userAgent', 'external_id']);
  ...
  const eventData = {
    ...
    event_id: eventId || `shenron_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
