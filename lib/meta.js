...
export async function getCampaigns(datePreset = 'maximum') {
  ...
  for (const campaign of (campaigns.data || [])) {
    ...
    // Fallback CPL when Meta omits cost_per_action_type for this result type:
    // spend / results is the same CPA. Un-blanks the active-campaign cpl and
    // last7d.cpl7d, and lets lead campaigns into topPerformers (which filters on
    // cpl !== null). Also sharpens the High-CPL alert below.
    if (cpl === null && results_count > 0) cpl = Math.round((spend / results_count) * 100) / 100;
...
// ── Campaign / Ad Set / Creative / Ad creation (Marketing API) ──
// Builds ad objects PAUSED by default. Activation stays a human action.
// Budgets/bids are dollars in, converted to cents.
// Graph wants the COMPLEX params (targeting, promoted_object,
// object_story_spec, special_ad_categories, creative) as JSON-STRING-encoded
// values even in an application/json body, same as the rule strings above.

const BID_CAP_STRATEGIES = new Set(['LOWEST_COST_WITH_BID_CAP', 'COST_CAP']);

// dollars -> integer cents, with a finite + minimum guard (Meta rejects sub-minimum budgets).
function toCents(dollars, label, min = 100) {
  const cents = Math.round(Number(dollars) * 100);
  if (!Number.isFinite(cents)) throw new Error(`${label}: invalid amount "${dollars}"`);
  if (cents < min) throw new Error(`${label}: ${cents} cents is below the ${min}-cent minimum`);
  return cents;
}

// Pass dailyBudgetDollars to make the campaign CBO (budget at campaign level);
// omit it for ABO (budget lives on each ad set).
export async function createCampaign({
  name, objective = 'OUTCOME_LEADS', status = 'PAUSED',
  specialAdCategories = [], dailyBudgetDollars = null,
  bidStrategy = 'LOWEST_COST_WITHOUT_CAP'
}) {
  if (!name) throw new Error('createCampaign: name required');
  const { adAccountId } = getConfig();
  const cbo = dailyBudgetDollars != null;
  const body = {
    name, objective, status,
    special_ad_categories: JSON.stringify(specialAdCategories || []),
    // This account requires the budget-sharing flag to be explicit on every
    // campaign. true = campaign budget shared across ad sets (CBO); false = ABO.
    is_adset_budget_sharing_enabled: cbo
  };
  if (cbo) {
    body.daily_budget = toCents(dailyBudgetDollars, 'createCampaign daily_budget');
    body.bid_strategy = bidStrategy;
  }
  return graphPost(`/${adAccountId}/campaigns`, body);
}

// Set campaignHasBudget=true when the parent campaign is CBO, so exactly one
// budget level is set. promotedObject e.g. { pixel_id, custom_event_type } or
// { pixel_id, custom_conversion_id }. targeting is required by Graph.
export async function createAdSet({
  name, campaignId, campaignHasBudget = false, dailyBudgetDollars = null,
  optimizationGoal = 'OFFSITE_CONVERSIONS', billingEvent = 'IMPRESSIONS',
  promotedObject = null, targeting = null, status = 'PAUSED',
  destinationType = null, bidStrategy = null, bidAmountDollars = null
}) {
  if (!name || !campaignId) throw new Error('createAdSet: name and campaignId required');
  if (!targeting) throw new Error('createAdSet: targeting required (Graph rejects an ad set with no targeting)');
  if (campaignHasBudget && dailyBudgetDollars != null) {
    throw new Error('createAdSet: parent campaign is CBO; do not also set an ad-set budget');
  }
  if (!campaignHasBudget && dailyBudgetDollars == null) {
    throw new Error('createAdSet: no budget set; pass dailyBudgetDollars (ABO) or set campaignHasBudget (CBO)');
  }
  if (optimizationGoal === 'OFFSITE_CONVERSIONS' && !promotedObject) {
    throw new Error('createAdSet: OFFSITE_CONVERSIONS requires a promotedObject (pixel + event)');
  }
  // Bid strategy lives at ad-set level for ABO; for CBO it lives on the campaign.
  // Default ABO ad sets to no-cap so no bid_amount is required (the account's
  // default strategy is a cap, which otherwise 400s subcode 2490487).
  const effectiveBidStrategy = campaignHasBudget ? bidStrategy : (bidStrategy || 'LOWEST_COST_WITHOUT_CAP');
  if (effectiveBidStrategy && BID_CAP_STRATEGIES.has(effectiveBidStrategy) && bidAmountDollars == null) {
    throw new Error(`createAdSet: ${effectiveBidStrategy} requires bidAmountDollars`);
  }
  const { adAccountId } = getConfig();
  const body = {
    name, campaign_id: campaignId,
    optimization_goal: optimizationGoal,
    billing_event: billingEvent, status,
    targeting: JSON.stringify(targeting)
  };
  if (dailyBudgetDollars != null) body.daily_budget = toCents(dailyBudgetDollars, 'createAdSet daily_budget');
  if (promotedObject) body.promoted_object = JSON.stringify(promotedObject);
  if (destinationType) body.destination_type = destinationType;
  if (effectiveBidStrategy) body.bid_strategy = effectiveBidStrategy;
  if (bidAmountDollars != null) body.bid_amount = toCents(bidAmountDollars, 'createAdSet bid_amount', 1);
  return graphPost(`/${adAccountId}/adsets`, body);
}

// Upload a video by hosted URL (e.g. Vercel Blob https URL). Uses its own
// longer timeout: Graph fetching a remote video can exceed the shared 15s.
// Returns { id } = video_id; poll getVideoStatus until status.video_status === 'ready'.
export async function uploadAdVideo({ fileUrl, name }) {
  if (!fileUrl) throw new Error('uploadAdVideo: fileUrl required');
  const { token, adAccountId } = getConfig();
  const body = { access_token: token, file_url: fileUrl };
  if (name) body.name = name;
  const resp = await fetch(`${GRAPH_API}/${adAccountId}/advideos`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
// Poll a freshly uploaded video. Returns { id, status: { video_status, processing_progress } }.
// Callers read result.status.video_status ('ready' | 'processing' | 'error').
export async function getVideoStatus(videoId) {
  return graphGet(`/${videoId}`, { fields: 'status' });
}

// Fetch a video's Meta-generated thumbnail URI (preferred frame if flagged).
export async function getVideoThumbnail(videoId) {
  const data = await graphGet(`/${videoId}/thumbnails`, {}).catch(() => null);
  const thumbs = data?.data || [];
  const pref = thumbs.find(t => t.is_preferred) || thumbs[0];
  return pref?.uri || null;
}

// Create a video ad creative. pageId is required; instagramActorId enables IG placements.
// thumbnailUrl is a public poster-frame URL; if omitted, Meta's own generated
// thumbnail is used (a video creative requires one, error_subcode 1443226).
export async function createVideoAdCreative({
  name, pageId, instagramActorId = null, videoId, thumbnailUrl = null,
  message, link, linkDescription = null, headline = null, ctaType = 'LEARN_MORE', displayLink = null
}) {
  if (!pageId || !videoId || !link) throw new Error('createVideoAdCreative: pageId, videoId, link required');
  const thumb = thumbnailUrl || await getVideoThumbnail(videoId);
  if (!thumb) throw new Error('createVideoAdCreative: no thumbnail available (video may still be processing)');
  // displayLink overrides the shown URL label (link_caption). Keep its root
  // domain matching the destination so Meta does not flag a mismatch.
  const ctaValue = { link };
  if (displayLink) ctaValue.link_caption = displayLink;
  const videoData = {
    video_id: videoId,
    image_url: thumb,
    message: message || '',
  ...
  if (linkDescription) videoData.link_description = linkDescription;
  ...
  return graphPost(`/${adAccountId}/adcreatives`, {
    name, object_story_spec: JSON.stringify(objectStorySpec)
  });
}

// Create an ad referencing an existing creative. PAUSED by default.
export async function createAd({ name, adsetId, creativeId, status = 'PAUSED' }) {
  if (!name || !adsetId || !creativeId) throw new Error('createAd: name, adsetId, creativeId required');
  const { adAccountId } = getConfig();
  return graphPost(`/${adAccountId}/ads`, {
    name, adset_id: adsetId, creative: JSON.stringify({ creative_id: creativeId }), status
  });
}

// Swap the creative on an existing ad (keeps the ad id and its paused/active state).
export async function updateAdCreative(adId, creativeId) {
  if (!adId || !creativeId) throw new Error('updateAdCreative: adId and creativeId required');
  return graphPost(`/${adId}`, { creative: JSON.stringify({ creative_id: creativeId }) });
}

// Set an ad's own status (ACTIVE / PAUSED). An ad only delivers when its own
// status AND its ad set AND its campaign are all ACTIVE.
export async function updateAdStatus(adId, status) {
  if (!adId || !status) throw new Error('updateAdStatus: adId and status required');
  return graphPost(`/${adId}`, { status });
}

// Delete any ad object (campaign, ad set, ad, creative) by id.
export async function deleteObject(id) {
  if (!id) throw new Error('deleteObject: id required');
  const { token } = getConfig();
  const url = `${GRAPH_API}/${id}?access_token=${encodeURIComponent(token)}`;
  const resp = await fetch(url, { method: 'DELETE', signal: AbortSignal.timeout(15000) });
  if (!resp.ok) {
    const body = await resp.text().catch(() => '');
    throw new Error(`Meta API DELETE ${resp.status}: ${body.slice(0, 300)}`);
  }
  return resp.json();
}

// Discover the Facebook Page + Instagram actor to attach to creatives.
// Primary source: an existing creative on the account. Fallback: promote_pages.
export async function getPromotePage() {
  const { adAccountId } = getConfig();
  let page = null;
  const creatives = await graphGet(`/${adAccountId}/adcreatives`, {
    fields: 'object_story_spec', limit: 25
  }).catch(() => null);
  const pid = (creatives?.data || [])
    .map(c => c?.object_story_spec?.page_id).find(Boolean);
  if (pid) page = { id: pid, source: 'existing-creative' };
  if (!page) {
    const promo = await graphGet(`/${adAccountId}/promote_pages`, { fields: 'id,name' }).catch(() => null);
    const p = promo?.data?.[0];
    if (p) page = { id: p.id, name: p.name, source: 'promote_pages' };
  }
  let instagramActorId = (creatives?.data || [])
    .map(c => c?.object_story_spec?.instagram_actor_id).find(Boolean) || null;
  if (!instagramActorId) {
    const ig = await graphGet(`/${adAccountId}/instagram_accounts`, { fields: 'id,username' }).catch(() => null);
    instagramActorId = ig?.data?.[0]?.id || null;
  }
  return { page, instagramActorId };
}
