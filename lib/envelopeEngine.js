...
export function computeBundle(envelope, selections) {
  ...
  if (bundleTier) {
    ...
    const remHard    = envelope?.components?.remediation?.hard || 0;
  ...
  const mob  = envelope?.standalone_mob_premium ?? DEFAULT_STANDALONE_MOB_PREMIUM;
  const mult = envelope?.standalone_multiplier  ?? DEFAULT_STANDALONE_MULTIPLIER;
  const remHard = (roofTier && envelope?.components?.remediation?.hard) ? envelope.components.remediation.hard : 0;
  const workHard = (roofTier?.hard || 0) + trimItems.reduce((s, t) => s + (t.hard || 0), 0);
  const sellingPre = roofTier ? Math.round((workHard + mob) * mult) : 0;
  return {
    mode: 'standalone_roof',
    tier: null,
    breakdown: {
      roof:   { hard: roofTier?.hard || 0, label: roofTier?.label || 'No roof selected', selling: roofTier ? computeStandalone(envelope, roofTier.hard || 0) : 0 },
      siding: null,
      wall:   null,
...
export function computeAloneTotal(envelope, selections) {
  ...
  if (roofTier && envelope?.components?.remediation?.hard) total += envelope.components.remediation.hard;
  return total;
}
