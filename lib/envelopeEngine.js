...
/**
 * Roof selling price. Asphalt tiers price the standard way (hard + mob, times
 * the multiplier). Metal tiers carry an explicit `price` built offline from the
 * SOP additive cost-stack divisor (metal labor is too high for the asphalt
 * multiplier to hold margin), so when `price` is present it IS the selling
 * price and the multiplier is bypassed.
 */
export function roofSelling(envelope, tier) {
  if (!tier) return 0;
  if (tier.price != null) return Math.round(tier.price);
  return computeStandalone(envelope, tier.hard || 0);
}
...
export function computeBundle(envelope, selections) {
  ...
  if (bundleTier) {
    ...
    // Only the WORK carries the package multiplier and is the discount base.
    // The refundable remediation allowance is reported separately and added at
    // face value after any cash discount (see computeEnvelope).
    const remComp    = envelope?.components?.remediation;
    const remHard    = (remComp?.hard && (!remComp.systems || remComp.systems.includes(system))) ? remComp.hard : 0;
    const workHard   = roofHard + wallHard + sidingHard + trimHard;
    const sellingPre = Math.round(workHard * bundleTier.mult);
    return {
      mode: 'bundle',
      tier: bundleTier,
      breakdown: {
        ...
        remediation: remHard ? { hard: remHard, label: envelope.components.remediation.label || 'Remediation Allowance' } : null
      },
      remediation: remHard,
      totalHard: workHard,
      sellingPre,
  ...
  // Standalone path: just roof (or roof + trim, no shell). Day-trip
  // mobilization is a single project cost, charged ONCE on the combined hard
  // cost (not once per line). Remediation is added at face value after markup.
  const mob  = envelope?.standalone_mob_premium ?? DEFAULT_STANDALONE_MOB_PREMIUM;
  const mult = envelope?.standalone_multiplier  ?? DEFAULT_STANDALONE_MULTIPLIER;
  // Remediation can be scoped via `systems` (Mary's metal road installs over
  // the existing roof — no tear-off → no remediation line).
  const remComp = envelope?.components?.remediation;
  const remHard = (roofTier && remComp?.hard && (!remComp.systems || remComp.systems.includes(system))) ? remComp.hard : 0;
  // A metal roof tier carries an explicit divisor `price` (travel baked in) and
  // bypasses the multiplier; asphalt hard cost + trim runs through it with one mob.
  const roofFixed = (roofTier && roofTier.price != null) ? Math.round(roofTier.price) : null;
  const workHard = (roofFixed != null ? 0 : (roofTier?.hard || 0)) + trimItems.reduce((s, t) => s + (t.hard || 0), 0);
  const multPart = (workHard > 0) ? Math.round((workHard + mob) * mult) : 0;
  const sellingPre = roofTier ? ((roofFixed != null ? roofFixed : 0) + multPart) : 0;
  return {
    mode: 'standalone_roof',
    tier: null,
    breakdown: {
      roof:   { hard: roofTier?.hard || 0, label: roofTier?.label || 'No roof selected', selling: roofSelling(envelope, roofTier) },
      siding: null,
      wall:   null,
...
export function computeAloneTotal(envelope, selections) {
  ...
  let total = 0;
  if (roofTier) total += roofSelling(envelope, roofTier);
  ...
  // Remediation is a flat pass-through: include it in the "separate price"
  // anchor so it reconciles with finalSelling (it cancels out of the savings
  // number, which nets it against the bundle base in computeEnvelope).
  const remComp = envelope?.components?.remediation;
  if (roofTier && remComp?.hard && (!remComp.systems || remComp.systems.includes(system))) total += remComp.hard;
  return total;
}
...
export function computeEnvelope(envelope, selections) {
  ...
  // Remediation is a flat pass-through included in alonePreTax; net it against
  // the bundle base so savings stays the pure bundling delta and the displayed
  // totals reconcile (Separate price - Savings - Discount = Investment).
  const remFace = bundle.remediation || 0;
  const savings = Math.max(0, alonePreTax - (bundle.sellingPre + remFace));
  ...
  // Refundable remediation allowance is added at face value AFTER the discount,
  // so a pay-in-full discount never reduces the credited-back amount.
  const finalSelling = (bundle.sellingPre - cashOff) + remFace;
