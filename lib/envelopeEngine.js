...
export function computeBundle(envelope, selections) {
  ...
  if (bundleTier) {
    ...
    // Remediation is a refundable allowance: charged at face value (added after
    // the markup) so the "credited back if unused" promise holds dollar-for-
    // dollar. Only the actual work carries the package multiplier.
    const remHard    = envelope?.components?.remediation?.hard || 0;
    const workHard   = roofHard + wallHard + sidingHard + trimHard;
    const sellingPre = Math.round(workHard * bundleTier.mult) + remHard;
    return {
      mode: 'bundle',
      tier: bundleTier,
      breakdown: {
  ...
  const sellingPre = roofTier ? Math.round((workHard + mob) * mult) + remHard : 0;
  return {
...
export function computeAloneTotal(envelope, selections) {
  ...
  // Remediation at face value (matches the charged allowance), once a roof is in play
