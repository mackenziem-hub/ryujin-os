...
export function reconcile({ estimates = [], workorders = [], paysheets = [], customers = [] } = {}, opts = {}) {
  ...
  const ledger = [];
  const findings = [];
  const costAsRevIdx = new Set(); // stable job indices flagged VALUE_LOOKS_LIKE_COST
  ...
  for (const j of jobs) {
    ...
    ledger.push({
      name: label, name_full: j.name || '', address: addrShort, state,
    ...
    const idx = ledger.length - 1; // stable job index for figure bucketing (R2)
  ...
  // Bucket figures by STABLE job index (R2) - never by the truncated display
  // label, which could collide for two distinct (correctly unmerged) jobs.
  let committedN = 0, deliveredN = 0, cancelledN = 0;
  let cleanContract = 0, noValueCount = 0, noValueSubCost = 0, reestimateSubCost = 0;
  ledger.forEach((l, i) => {
    if (l.state === 'cancelled') { cancelledN++; return; }
    if (l.state === 'committed') committedN++; else deliveredN++;
    const costAsRev = costAsRevIdx.has(i);
    if (l.contract > 0 && !costAsRev) cleanContract += l.contract;
    if (l.contract <= 0) { noValueCount++; noValueSubCost += l.sub_cost; }
    if (l.contract <= 0 || costAsRev) reestimateSubCost += l.sub_cost;
  });
  const activeN = committedN + deliveredN;
  ...
  return {
    ...
    figures: {
      ...
      committed_delivered_jobs: activeN,
      committed: committedN,
      delivered: deliveredN,
      cancelled: cancelledN,
      contract_value_clean: round(cleanContract),
      jobs_with_no_recorded_contract_value: noValueCount,
      cost_as_revenue_jobs: costAsRevIdx.size,
      no_value_sub_cost_floor: round(noValueSubCost),
...
// ═══════════════════════════════════════════════════════════════
// SENTINEL CHECKS (the money-bug scans, per Mac Jun 12: "effective agent
// scans are the solution"). Pure function so it unit-tests without env.
// Folded into the daily reconcile agent run; findings ride the same
// reconciliation_findings dedup/dismiss/auto-resolve machinery.
// ═══════════════════════════════════════════════════════════════
import { dedupeEstimatePool } from './paymentMatcher.js';

export function sentinelChecks({ pool = [], cashflow = null, paysheets = [], now = Date.now() } = {}) {
  const findings = [];
  const add = (kind, job, detail, fix, dollar = null, extra = {}) =>
    findings.push({ kind, job, detail, proposed_fix: fix, dollar_impact: dollar, ...extra });

  // 1. Duplicate customer rows across estimate sources. PR #394 made the
  // cashflow CONSUMER immune, but the duplicate data itself still drifts
  // (pre-tax vs tax-in totals on the same customer). Flag each name once.
  if (pool.length) {
    const counts = new Map();
    for (const e of pool) {
      const k = (e.fullName || '').toLowerCase().trim();
      if (!k) continue;
      counts.set(k, (counts.get(k) || 0) + 1);
    }
    const deduped = dedupeEstimatePool(pool);
    if (deduped.length < pool.length) {
      for (const [name, count] of counts) {
        if (count > 1) {
          add('dup_estimate_rows', name,
            `${count} estimate rows across sources for the same customer; totals may disagree (pre-tax vs tax-in basis)`,
            'Confirm which row is canonical; align or archive the other');
        }
      }
    }
  }

  // 2. Cashflow matcher exceptions (computed by api/agents/cashflow.js since
  // PR #392). Persisting them here gives each one dedup + dismiss + history
  // instead of living only in the latest snapshot.
  const cf = cashflow || {};
  for (const x of (cf.arExceptions || [])) {
    if (x.type === 'unmatched_large') {
      add('payment_unmatched', x.customer,
        `$${Number(x.amount).toFixed(2)} payment matches no estimate ("${x.description || 'no descriptor'}")`,
        'Tie it to a job or record it as other income', Number(x.amount));
    } else if (x.type === 'duplicate_suspect') {
      add('payment_duplicate_suspect', x.customer,
        `two $${Number(x.amount).toFixed(2)} payments within 60 seconds`,
        'Check the bank ledger for a double charge', Number(x.amount));
    }
    // over_collected is recomputed from byJob below so the check still fires
    // on snapshots that predate the arExceptions deploy.
  }

  // 4. Contract-vs-collected drift (the Fram class): payments exceed the
  // recorded contract. A bookkeeping entry is needed either way (change
  // order, refund, or contract correction).
  for (const j of (cf.byJob || [])) {
    if (j.contractValue && j.totalCollected > j.contractValue + 1) {
      const excess = Math.round((j.totalCollected - j.contractValue) * 100) / 100;
      const extra = (typeof j.estimateId === 'string' && /^[0-9a-f-]{36}$/i.test(j.estimateId))
        ? { estimate_id: j.estimateId } : {};
      add('collected_over_contract', j.customer,
        `collected $${Number(j.totalCollected).toFixed(2)} against recorded contract $${Number(j.contractValue).toFixed(2)}`,
        'Bookkeeping entry needed: change order, refund, or contract correction', excess, extra);
    }
  }

  // 3. Phantom-AP class: paysheet never marked paid although the work
  // finished more than 14 days ago. FLAG ONLY, never auto-close: settling
  // out-of-band is Mac's call (see the Jun 12 Atlantic close-out).
  const cutoff = now - 14 * 86400000;
  for (const p of paysheets) {
    if (p.paid_at) continue;
    const status = String(p.status || '');
    if (status !== 'completed' && status !== 'invoice_final') continue;
    const doneAt = Date.parse(p.completed_at || p.completed_date || '') || null;
    if (doneAt && doneAt < cutoff) {
      const days = Math.floor((now - doneAt) / 86400000);
      add('unpaid_aged_paysheet', p.customer_name || p.address || String(p.id),
        `paysheet $${Number(p.total).toFixed(2)} completed ${days}d ago, still not marked paid in the system`,
        'Confirm whether settled out-of-band; close out with paid_at + paid_to_date (never auto-close)',
        Number(p.total) || null);
    }
  }
