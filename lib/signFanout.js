...
export function planSignFanout(job = {}, people = {}) {
  ...
  const artifacts = [
    {
      // v1: a paysheets ROW needs a job_id (project) + the measured labour, neither
      // present at the instant of signing. So the sub pay fires as a draft task to
      // the office (Cat) to re-rate it to the measurements (feedback_paysheet_eagleview_basis).
      // Title is generic (subcontractor) so it works for roofing (Ryan) AND Revive
      // (the sprayer) the same way -- Cat assigns the right sub. Auto-creating the
      // paysheet row is phase 2 (once the job/project + labour exist).
      kind: 'task', to: 'Cat (sub paysheet)', to_id: people.cat || null,
      table: 'tickets',
      fields: {
        title: `Draft the subcontractor paysheet: ${jobLabel}`,
        assigned_to: people.cat || null, priority: 'high', status: 'open',
        estimate_id: estId,
        description: `${cust} signed${total != null ? ` ($${total.toLocaleString()})` : ''}. Draft the subcontractor paysheet, labour re-rated to the measurements (NOT the customer price). Address: ${addr}.`,
      },
      note: 'paysheet fires as a draft-from-measurements task (a paysheet row needs a job_id + labour not present at sign); auto-create is phase 2',
    },
    {
