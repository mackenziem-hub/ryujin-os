...
export async function executeSignFanout(job, people, opts = {}) {
  ...
  // Idempotency: if a "Schedule the job:" task already exists for this estimate,
  // the fan-out already ran -> skip (a re-accept must not double-fire).
  if (estId) {
    try {
      const r = await f(`${baseUrl}/api/tickets?estimate_id=${encodeURIComponent(estId)}`, { headers });
      if (r.ok) {
        const j = await r.json().catch(() => null);
        const arr = (j && (j.tickets || j.items)) || (Array.isArray(j) ? j : []);
        if (Array.isArray(arr) && arr.some(t => /^Schedule the job:/.test(t.title || ''))) {
          return { created: [], skipped: 'already fanned out for this estimate', estId };
        }
      }
