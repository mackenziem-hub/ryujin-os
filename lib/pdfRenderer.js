// Ryujin OS — Server-side PDF renderer for work orders + pay sheets
// Mirrors public/assets/ryujin-pdf.js (jsPDF) so server can emit the same documents
// via /api/workorders-pdf and /api/paysheets-pdf.
import { jsPDF } from 'jspdf';
import { DEFAULT_BRAND } from './brandDefaults.js';

const SENSITIVE_DEFAULTS = ['$','total','subtotal','HST','GST','tax','financ','deposit','balance','commission','payout','margin','quote','estimate','price','cost','retail'];

function buildSensitiveRx(terms){
  const list = (terms && terms.length) ? terms : SENSITIVE_DEFAULTS;
...
// brand: white-label override point (PR3). Callers that pass nothing get the
// platform default (tenant #1's identity), byte-identical to the old literals.
function header(doc, title, subtitle, brand = DEFAULT_BRAND){
  ...
  doc.setFontSize(18);
  doc.text(brand.pdfHeaderName || DEFAULT_BRAND.pdfHeaderName, 40, 32);
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(200, 220, 255);
  doc.text(brand.pdfHeaderLine || DEFAULT_BRAND.pdfHeaderLine, 40, 48);
  doc.setTextColor(251, 146, 60);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.text(String(title || '').toUpperCase(), 612 - 40, 32, { align: 'right' });
  if (subtitle){
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    doc.setTextColor(150, 180, 210);
    doc.text(String(subtitle), 612 - 40, 48, { align: 'right' });
  }
...
function footer(doc, refId){
  ...
  doc.text(`Generated ${d} · Ryujin OS${refId ? ' · ' + refId : ''}`, 40, 770);
...
function sectionTitle(doc, y, text){
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.text(String(text).toUpperCase(), 50, y + 7);
...
// ─── WORK ORDER ───────────────────────────────────────────────
export function renderWorkOrderPDF(wo, opts = {}){
  const SENSITIVE_RX = buildSensitiveRx(opts.sensitiveTerms);
  const doc = newDoc();
  header(doc, 'Work Order', 'WO-' + (wo.wo_number || '—'), opts.brand || DEFAULT_BRAND);
  let y = 90;
  ...
  y = row(doc, y, 'Status', String(wo.status || 'draft').replace('_',' ').toUpperCase());
  if (wo.sub_crew_lead) y = row(doc, y, 'Crew Lead', wo.sub_crew_lead);

  y += 6;
  y = sectionTitle(doc, y, 'Scope');
  y = row(doc, y, 'Job Type', String(wo.job_type || 'full_replacement').replace('_',' '));
  ...
  y = row(doc, y, 'Tier', String(wo.package_tier || '—').toUpperCase());
  ...
  if (wo.scope_items && wo.scope_items.length) {
    ...
    wo.scope_items.map(s => typeof s === 'string' ? { item: s, included: true } : s).forEach(s => {
  ...
  if (wo.special_notes && !SENSITIVE_RX.test(wo.special_notes)) {
    y += 4;
  ...
  if (wo.notes && !SENSITIVE_RX.test(wo.notes)) {
    y += 4;
...
// ─── PAY SHEET ────────────────────────────────────────────────
export function renderPaysheetPDF(ps, opts = {}){
  const doc = newDoc();
  header(doc, 'Subcontractor Pay Sheet', safe(ps.job_id), opts.brand || DEFAULT_BRAND);
  let y = 90;
  ...
  y = row(doc, y, 'Status', String(ps.status || 'scheduled').replace('_',' ').toUpperCase());
