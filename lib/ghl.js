...
// GHL returns timestamps inconsistently: /conversations/search gives
// `lastMessageDate` as epoch MILLIS (a number or an all-digit string), while
// other endpoints use ISO strings. A bare epoch-ms value written straight into
// a timestamptz column throws Postgres "date/time field value out of range"
// (which silently dropped every inbox_items insert the first time the agent
// ran live). Normalize everything to ISO at the GHL boundary.
export function ghlDateToIso(v){
  if (v == null || v === '') return null;
  const s = String(v).trim();
  if (typeof v === 'number' || /^\d{9,}$/.test(s)) {
    let n = Number(v);
...
export async function getConversationMessages(conversationId, { limit = 25 } = {}){
  ...
  return raw.map(m => ({
    ...
    status: m.status,
    dateAdded: ghlDateToIso(m.dateAdded),
  }));
}
...
// Find a contact in this location by phone number. Returns the best match
// { id, name, phone, email } or null. Used to resolve the owner SMS target at
// runtime instead of hardcoding a contact id — a stale hardcoded id failed
// once with GHL 400 "Contact not found". Matches on the last 10 digits so
// formatting (+1, spaces, dashes, parens) does not matter.
export async function getContactByPhone(phone){
  const { locationId } = getConfig();
  const digits = String(phone || '').replace(/\D/g, '');
  if (digits.length < 10) return null;
  const last10 = digits.slice(-10);
  const data = await ghlFetch('/contacts/', { query: { locationId, query: last10, limit: '20' } });
  const contacts = data?.contacts || data?.data || (Array.isArray(data) ? data : []);
  const c = contacts.find(x => String(x.phone || '').replace(/\D/g, '').endsWith(last10)) || null;
  if (!c) return null;
  return {
    id: c.id,
