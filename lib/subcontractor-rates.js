...
// Canonical source: Plus Ultra/Production/SUBCONTRACTOR_RATE_SHEET_v2_2026.md
// (LOCKED Apr 30 2026, restored to engine May 8 2026 after Apr 28 actualization
// drift was caught on Royal Oaks duplex quote review.
// May 8 2026 PM: travel surcharge converted from stepped brackets to per-km linear
// after Brian Dorken #39 customer pushback exposed the 60-km cliff was charging
// $380 retail jumps for tenths-of-a-kilometer changes. New formula: $1/SQ per km
// above 40 km free zone. Matches old 40-60 band exactly at the boundary, smooth
// from there. Waste removal flat bands kept as-is.)
export const RATE_SHEET_VERSION = '2025_v2.2_perkm_2026-05-08';
...
export const SUB_RATES = {
  'atlantic-roofing': {
    ...
    rate_sheet_source: 'Plus Ultra v2.1 LOCKED canonical (Apr 30 2026) — Plus Ultra/Production/SUBCONTRACTOR_RATE_SHEET_v2_2026.md',

    // Base labor per SQ, by pitch tier — v2.1 canonical (Apr 30 2026)
    // Includes tear-off of 1 existing layer + underlayment + ice & water at eaves
    // (NB code minimum) + starter + field shingles + ridge cap + standard cleanup.
    base_per_sq: {
      '4-6':    130,   // walkable
      '7-9':    160,   // steep
      '10-12':  190,   // very steep
      '13+':    200,   // mansard / harness required
      'mansard': 200
    },

    // Tear-off + decking — v2.1 canonical
    // Multi-layer comp 3+: $50/SQ/additional layer (handled separately, see scope_extras)
    extra_layer_per_sq: 40,                  // v2.1: $40/SQ per ADDITIONAL layer beyond the 1 included in base
    deck_pu_supplied_per_sheet: 30,          // Mac supplies sheet, sub installs
    deck_sub_supplied_per_sheet: 60,         // sub supplies + installs ($30 install + $30 material)
    ...
    step_flash_per_lf: 1.50,
    wall_flash_per_lf: 1.50,

    // Penetrations — chimney brackets per v2.1 Section 1.10
    // Naming kept as flue-count keys for backward compat, but mapped to v2.1 brackets:
    //   single_flue → small/medium chimney ($150)
    //   double_flue → large / 2-side chimney ($200)
    //   triple_flue → custom / grinded ($300)
    //   stone/extreme custom: quoted separately, not auto-priced ($500+ written quote)
    chimney_flash_single_flue: 150,          // v2.1: small/medium chimney
    chimney_flash_double_flue: 200,          // v2.1: large or 2-side chimney
    chimney_flash_triple_flue: 300,          // v2.1: custom/grinded chimney
    chimney_flash_steel: 75,                 // v2.1: rooftop chimney cap install (storm cap)
    skylight_install_new: 150,               // v2.1: new skylight install (deck cut)
    skylight_reuse: 75,                      // v2.1: reflash walkable pitch
    skylight_reuse_steep: 125,               // v2.1: reflash steep (7/12+)
    skylight_full_replacement: 500,          // v2.1: full Velux FS/VS/VCM replacement
    pipe_boot_each: 25,
    cut_in_vent_each: 15,
    accessory_bundle_install: 25,
    bundle_carry_up: 2,
    ...
    // Travel surcharge — v2.2 per-km linear (May 8 2026)
    // Formula: $1/SQ per km above 40 km free zone. Below 40 km = $0.
    // Matches old 40-60 bracket exactly at 60 km (= $20/SQ), smooths from there.
    // Replaces stepped brackets that created cliffs at the 60-km boundary.
    travel_per_sq_per_km_above_40: 1.00,
    travel_free_zone_km: 40,
    // Legacy bracket fields kept for any caller still referencing them (deprecated):
...
// Per-SQ travel surcharge — v2.2 per-km linear (May 8 2026)
// Formula: $rate × max(0, distanceKm − freeZone)
// Default rate $1/SQ/km above 40 km free zone. Tunable via rate sheet for per-sub overrides.
// Replaces v2.1 stepped brackets (40-60: $20/SQ, 60+: $30/SQ) which created
// $380 retail cliffs for tenths-of-a-km distance changes.
export function pickTravelPerSQ(rates, distanceKm) {
  const km = Number(distanceKm) || 0;
  const freeZone = Number(rates.travel_free_zone_km ?? 40);
  const perKm = Number(rates.travel_per_sq_per_km_above_40 ?? 1.00);
  return Math.max(0, km - freeZone) * perKm;
}
...
export function computeSubPaysheet(measurements = {}, package_tier = null, scope_extras = {}, sub_slug = 'atlantic-roofing') {
  ...
  // Multi-plane base labor (May 7 2026): when m.planes is provided, split base
  // labor into per-plane lines at each plane's pitch band rate. Each plane's
  // `sq` is already pitch-adjusted SQ (engine computes it from raw 2D × pitch
  // multiplier ÷ 100). If the per-plane SQs sum to less than totalSQ, the
  // remainder is billed at the dominant pitch tier so we don't underbill —
  // catches small areas the user didn't enumerate (overhangs, return corners).
  const inputPlanes = Array.isArray(m.planes) ? m.planes.filter(p => p && Number(p.sq) > 0 && p.pitch) : [];

  if (inputPlanes.length > 0) {
    let planeSqSum = 0;
    for (const p of inputPlanes) {
      const psq = Number(p.sq) || 0;
      const pTier = pickPitchTier(p.pitch);
      const pRate = rates.base_per_sq[pTier];
      if (!pRate) continue;
      const lbl = p.label
        ? `Base labor — ${p.label} (${p.pitch}, ${pTier} band)`
        : `Base labor — ${p.pitch} (${pTier} band)`;
      labour_breakdown.push(line(lbl, psq, 'SQ', pRate, psq * pRate));
      planeSqSum += psq;
    }
    // Remainder catch-all at dominant pitch — only fires if planes underspecify.
    const remainder = Math.max(0, totalSQ - planeSqSum);
    if (remainder > 0) {
      labour_breakdown.push(line(
        `Base labor — additional area (${pitchTier} band)`,
        remainder, 'SQ', basePerSQ, remainder * basePerSQ
      ));
    }
  } else {
    labour_breakdown.push(line(
  ...
  // Chimney flashing — v2.1 §1.10: single/small/medium $150, double/large/2-side $200,
  // triple/custom/grinded $300, steel rooftop cap $75. Fallbacks below mirror the canonical
  // DEFAULTS rate sheet so a partial/missing rate object can't silently underprice.
  // Backward compat: 'small'/'medium' size → single flue, 'large' → double, 'triple'/'custom'/'grinded' → triple.
  function chimneyRate(sizeOrFlueCount) {
    const k = String(sizeOrFlueCount || 'single').toLowerCase();
    if (k === 'steel' || k === 'metal') return rates.chimney_flash_steel ?? 75;
    if (k === 'triple' || k === '3' || k === 'custom' || k === 'grinded') return rates.chimney_flash_triple_flue ?? 300;
    if (k === 'double' || k === 'large' || k === '2' || k === '2-side' || k === '2-sided') return rates.chimney_flash_double_flue ?? 200;
    return rates.chimney_flash_single_flue ?? 150;
  }
  ...
  // Waste removal — flat by distance band, OR explicit override for big jobs
  // (e.g. duplex teardowns needing 3 dump runs). When measurements.waste_removal_override
  // is set (number > 0), use that value instead of the band rate. Default behaviour
  // unchanged for typical single-family jobs.
  const wasteOverride = Number(m.waste_removal_override) || 0;
  if (wasteOverride > 0) {
    surcharges.push({
      label: `Waste removal (multi-load, override)`,
      total: round2(wasteOverride)
    });
  } else {
