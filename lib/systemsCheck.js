...
export async function runSystemsCheck() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // Read the canonical rebuild stamp (last_full_rebuild_at), not updated_at:
      // partial agent POSTs keep updated_at minutes-fresh regardless of whether
      // the live sections actually rebuilt. With the */15 rebuild cron, a true
      // rebuild older than 60 min is a real health problem worth flagging.
      const enriched = d.last_full_rebuild_at || d.updated_at || d.enrichedAt || d.timestamp || null;
      const ageMin = enriched ? Math.round((Date.now() - new Date(enriched).getTime()) / 60000) : null;
      checks.snapshot = {
        ok: ageMin != null && ageMin < 60,
        ageMinutes: ageMin,
        enrichedAt: enriched
      };
    } else {
