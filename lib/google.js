...
function buildMimeMessage(to, subject, body, options = {}) {
  const { cc, bcc, inReplyTo, references, from, replyTo, html } = options;
  // Strip CR/LF from header values to prevent header injection. RFC 2047 encode
  // values carrying non-ASCII (accented names etc.) so they don't show as mojibake.
  const h = (v) => String(v ?? '').replace(/[\r\n]+/g, ' ');
  const enc = (v) => /[^\x00-\x7F]/.test(v) ? `=?UTF-8?B?${Buffer.from(v, 'utf-8').toString('base64')}?=` : v;
  const lines = [];
  if (from) lines.push(`From: ${h(from)}`);
  lines.push(`To: ${h(to)}`);
  if (cc) lines.push(`Cc: ${h(cc)}`);
  if (bcc) lines.push(`Bcc: ${h(bcc)}`);
  if (replyTo) lines.push(`Reply-To: ${h(replyTo)}`);
  lines.push(`Subject: ${enc(h(subject))}`);
  if (inReplyTo) {
    lines.push(`In-Reply-To: ${h(inReplyTo)}`);
    lines.push(`References: ${h(references || inReplyTo)}`);
  }
  if (html) {
    // multipart/alternative: plain-text fallback + HTML. Random boundary so it
    // cannot collide with the message content.
    const boundary = `=_ryujin_${crypto.randomBytes(12).toString('hex')}`;
