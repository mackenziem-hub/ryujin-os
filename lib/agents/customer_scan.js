...
export async function runCustomerScan({ tenantSlug = 'plus-ultra' } = {}) {
  ...
  const ests = await supabaseAdmin
    .from('estimates')
    .select('id, estimate_number, status, state, proposal_mode, share_token, created_at, updated_at, approved_at, schedule_due_by, scheduled_at, closed_won_at, customer:customers(full_name, email, phone)')
  ...
  // Heuristic: closed_won_at is N+ days old. Once we add a `review_request_sent_at` column we can be exact.
  const wonOld = list.filter(e => {
    if (e.state !== 'closed_won' && e.status !== 'signed') return false;
    const closedAt = e.closed_won_at ? new Date(e.closed_won_at).getTime() : (e.updated_at ? new Date(e.updated_at).getTime() : 0);
    if (!closedAt) return false;
    const days = (now - closedAt) / 86400000;
    return days >= SIGNED_NO_REVIEW_DAYS && days <= SIGNED_NO_REVIEW_DAYS + 30;
  });
