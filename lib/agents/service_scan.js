...
// Defaults only. Per-tenant overrides come from tenant_settings.service_config
// (saved by /service-admin.html via /api/settings): sla.overdue_grace_hours,
// sla.ack_hours, behaviors.callback_threshold_pct.
const OVERDUE_HOURS = 24;             // grace after scheduled_at before flagging overdue
const CALLBACK_THRESHOLD_PCT = 8;     // 12mo callback-rate alert threshold
...
export async function runServiceScan({ tenantSlug = 'plus-ultra' } = {}) {
  ...
  // ── Tenant service config (SLA hours + thresholds, owner-tunable) ──
  let svcConfig = {};
  const cfg = await supabaseAdmin
    ...
    .maybeSingle();
  if (!cfg.error && cfg.data?.service_config) svcConfig = cfg.data.service_config;
  const num = (v, dflt) => { const n = Number(v); return Number.isFinite(n) && n > 0 ? n : dflt; };
  const overdueHours = num(svcConfig.sla?.overdue_grace_hours, OVERDUE_HOURS);
  const ackHours = num(svcConfig.sla?.ack_hours, OVERDUE_HOURS);
  const callbackThreshold = num(svcConfig.behaviors?.callback_threshold_pct, CALLBACK_THRESHOLD_PCT);
  ...
  const stats = report.stats = {
    ...
    tickets_overdue: 0,
    tickets_unacknowledged: 0,          // open + no acknowledged_at past ack SLA
  ...
  for (const t of tickets) {
    if (t.status === 'open') {
      ...
      if (t.scheduled_at) {
        ...
        if (sched < now - overdueHours * 3600000) {
  ...
  // ── SLA: unacknowledged open tickets past sla.ack_hours ──
  // Separate query so a missing acknowledged_at column (migration 097 not
  // applied yet) degrades to a loud finding instead of killing the scan.
  const ackBreaches = [];
  const ackCutoff = new Date(now - ackHours * 3600000).toISOString();
  const ack = await supabaseAdmin
    .from('service_tickets')
    .select('id, title, reported_at, acknowledged_at, customer:customers(full_name)')
    .eq('tenant_id', tenantId)
    .eq('status', 'open')
    .is('acknowledged_at', null)
    .lt('reported_at', ackCutoff)
    .order('reported_at', { ascending: true })
    .limit(100);
  if (ack.error) {
    report.findings.push(`Ack-SLA check unavailable: ${ack.error.message} (apply migration 097)`);
  } else {
    stats.tickets_unacknowledged = (ack.data || []).length;
    ackBreaches.push(...(ack.data || []));
  }
  ...
  if (stats.tickets_unacknowledged > 0) {
    const n = stats.tickets_unacknowledged;
    report.findings.push(`${n} open ticket${n === 1 ? '' : 's'} unacknowledged past the ${ackHours}h ack SLA`);
    report.tasks.push({
      title: `Acknowledge ${n} ticket${n === 1 ? '' : 's'} past the ${ackHours}h ack SLA`,
      description: ackBreaches.slice(0, 8).map(t => `• ${t.customer?.full_name || 'unknown'} · ${t.title} · reported ${t.reported_at}`).join('\n'),
  ...
  if (stats.callback_rate_pct >= callbackThreshold && stats.tickets_complete_12mo >= 20) {
    report.findings.push(`Callback rate ${stats.callback_rate_pct}% is above the ${callbackThreshold}% threshold, investigate crew workmanship or material defects`);
    report.tasks.push({
      title: `Review callback drivers, rate is ${stats.callback_rate_pct}% (threshold ${callbackThreshold}%)`,
