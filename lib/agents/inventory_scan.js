...
export async function runInventoryScan({ tenantSlug = 'plus-ultra' } = {}) {
  ...
  // ── Purchase orders (migration 063 — soft-fails before applied) ──
  let overduePos = [];
  let staleDrafts = [];
  try {
    const todayDate = new Date().toISOString().slice(0, 10);
    const allActive = await supabaseAdmin
      .from('purchase_orders')
      .select('id, po_number, status, expected_delivery_date, created_at, ordered_at, total_amount, merchant:merchants(name)')
    ...
    if (!allActive.error) {
      const rows = allActive.data || [];
      stats.po_open = rows.length;
      stats.po_draft = rows.filter(p => p.status === 'draft').length;
      stats.po_sent = rows.filter(p => p.status === 'sent').length;
      stats.po_confirmed = rows.filter(p => p.status === 'confirmed').length;

      for (const p of rows) {
        if (p.expected_delivery_date && p.expected_delivery_date < todayDate
            && ['sent', 'confirmed', 'partial'].includes(p.status)) {
          overduePos.push(p);
        }
        if (p.status === 'draft' && p.created_at) {
          const daysOld = (now - new Date(p.created_at).getTime()) / 86400000;
          if (daysOld >= 7) staleDrafts.push({ ...p, days_old: Math.floor(daysOld) });
        }
      }
      stats.po_overdue = overduePos.length;
      stats.po_drafts_stale = staleDrafts.length;
    }
  } catch { /* purchase_orders table doesn't exist yet — migration 063 pending */ }

  if (overduePos.length > 0) {
    report.findings.push(
      `${overduePos.length} purchase order${overduePos.length === 1 ? '' : 's'} overdue (expected delivery date passed, still in flight)`
    );
    report.tasks.push({
      title: `Chase ${overduePos.length} overdue purchase order${overduePos.length === 1 ? '' : 's'}`,
      description: overduePos.slice(0, 6).map(p => `• ${p.po_number} · ${p.merchant?.name || 'supplier'} · expected ${p.expected_delivery_date}`).join('\n'),
      priority: overduePos.length >= 3 ? 'high' : 'medium'
    });
  }

  if (staleDrafts.length > 0) {
    report.findings.push(
      `${staleDrafts.length} purchase order${staleDrafts.length === 1 ? '' : 's'} sitting in draft 7+ days — may be forgotten`
    );
    report.tasks.push({
      title: `Send or cancel ${staleDrafts.length} stale draft PO${staleDrafts.length === 1 ? '' : 's'}`,
      description: staleDrafts.slice(0, 6).map(p => `• ${p.po_number} · ${p.merchant?.name || 'supplier'} · ${p.days_old}d old`).join('\n'),
      priority: 'normal'
    });
  }
