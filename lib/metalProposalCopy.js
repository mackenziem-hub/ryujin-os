...
// Canonical install rank for a metal slug, collapsing the DB-offer aliases
// (metal-americana / metal-standing-seam) onto the rank keys
// (metal-standard / metal-enhanced) so a template that requests one alias and an
// estimate that stored the other still match. Returns 'standard' | 'enhanced' |
// 'premium' for any known metal slug, or null for a non-metal / unknown slug.
export function metalRankOf(slug) {
  const copy = getMetalCopy(slug);
  return copy ? copy.rank : null;
}
