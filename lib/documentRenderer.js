...
export function renderSalesPageHTML(salesData) {
  ...
  const coverPhoto = s.coverPhoto || (s.hero?.photos && s.hero.photos.length > 0 ? s.hero.photos[0] : '');
  ...
  <!-- ═══ SLIDE 10: CTA — See your pricing ═══ -->
  <div class="cta-section">
    <h2>${esc(customerFirst)}, Ready to Move Forward?</h2>
    <div class="sub">Your complete ${esc(packageName)} proposal is below. Pricing, scope, warranty. Everything you need to make a decision.</div>
    <a href="#proposal" class="cta-btn" onclick="document.getElementById('proposal').scrollIntoView({behavior:'smooth'});return false;">See Your Investment Breakdown</a>
    <a href="tel:${esc((rep.phoneTel||contactPhone).replace(/[^0-9]/g,''))}" class="cta-alt">Or call ${esc(rep.name.split(' ')[0])} directly at ${esc(contactPhone)}</a>
    <div class="financing">Financing available through FinanceIt &bull; As low as $99/mo</div>
  </div>

  <!-- ═══ EMBEDDED PROPOSAL — Pricing Section ═══ -->
  <div id="proposal" style="scroll-margin-top:20px;padding-top:40px">
  ${(() => {
    const p = s._proposalData;
    if (!p || !p.lineItems || p.lineItems.length === 0) {
      // Fallback: use the pricing from the sales page data itself
      const sp = s.pricing;
      if (!sp) return '';
      return `
      <div style="max-width:800px;margin:0 auto;padding:40px 24px">
        <div style="text-align:center;margin-bottom:40px">
          ...
          <h2 style="font-family:'Montserrat',sans-serif;font-size:2.4em;font-weight:900;color:#fff;margin-bottom:8px">${esc(sp.total || sp.totalFormatted || '')}</h2>
          <div style="font-size:1em;color:rgba(255,255,255,0.5)">${esc(packageName)} Package &bull; ${esc(sp.taxLabel || 'HST')} included</div>
        </div>
        ${sp.subtotal ? `<div style="max-width:400px;margin:0 auto">
          <div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:0.95em"><span style="color:rgba(255,255,255,0.5)">Subtotal</span><span style="color:#fff;font-weight:600">${esc(sp.subtotal)}</span></div>
          <div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:0.95em"><span style="color:rgba(255,255,255,0.5)">${esc(sp.taxLabel||'HST')}</span><span style="color:#fff;font-weight:600">${esc(sp.tax || '')}</span></div>
          <div style="display:flex;justify-content:space-between;padding:16px 0;font-size:1.1em;border-top:2px solid rgba(255,255,255,0.15);margin-top:4px"><span style="color:#fff;font-weight:800">Total</span><span style="color:#fff;font-weight:900;font-family:'Montserrat',sans-serif;font-size:1.2em">${esc(sp.total || '')}</span></div>
        </div>` : ''}
    ...
    return `
    <div style="max-width:800px;margin:0 auto;padding:80px 24px 40px">
      <div style="text-align:center;margin-bottom:40px">
        <div style="font-size:0.72em;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:rgba(255,255,255,0.35);margin-bottom:12px">Your Investment</div>
        <h2 style="font-family:'Montserrat',sans-serif;font-size:2.4em;font-weight:900;color:#fff;margin-bottom:8px">${esc(p.pricing?.total || '')}</h2>
        <div style="font-size:1em;color:rgba(255,255,255,0.5)">${esc(p.proposal?.offerName || packageName)} Package &bull; ${esc(p.pricing?.taxLabel || 'HST')} included</div>
      </div>
      ...
      <div style="max-width:500px;margin:0 auto">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:20px 0 8px;margin-top:8px">
          <div style="font-size:0.85em;color:rgba(255,255,255,0.5)">Subtotal</div>
          <div style="font-family:'Montserrat',sans-serif;font-weight:600;color:rgba(255,255,255,0.7)">${esc(p.pricing?.subtotal || '')}</div>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0">
          <div style="font-size:0.85em;color:rgba(255,255,255,0.5)">${esc(p.pricing?.taxLabel || 'HST')}</div>
          <div style="font-family:'Montserrat',sans-serif;font-weight:600;color:rgba(255,255,255,0.7)">${esc(p.pricing?.tax || '')}</div>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;padding:20px 0;border-top:2px solid rgba(255,255,255,0.15);margin-top:8px">
          <div style="font-size:1.1em;font-weight:800;color:#fff">Total</div>
          <div style="font-family:'Montserrat',sans-serif;font-weight:900;font-size:1.4em;color:#fff">${esc(p.pricing?.total || '')}</div>
        </div>
      </div>

      ${p.warranty ? `<div style="text-align:center;margin-top:32px;padding:20px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px;font-size:0.88em;color:rgba(255,255,255,0.6)">${esc(p.warranty)}</div>` : ''}

      <div style="text-align:center;margin-top:40px">
        <a href="tel:${esc((rep.phoneTel||contactPhone).replace(/[^0-9]/g,''))}" style="display:inline-block;padding:16px 48px;background:${esc(accent)};color:#fff;border-radius:10px;font-family:'Montserrat',sans-serif;font-weight:800;font-size:1.05em;text-decoration:none;letter-spacing:0.5px;transition:transform 0.2s" onmouseover="this.style.transform='scale(1.03)'" onmouseout="this.style.transform='none'">Let's Do This — Call ${esc(rep.name.split(' ')[0])}</a>
        <div style="margin-top:16px;font-size:0.85em;color:rgba(255,255,255,0.35)">Financing available through FinanceIt &bull; Valid for 30 days</div>
      </div>
    </div>`;
  })()}
