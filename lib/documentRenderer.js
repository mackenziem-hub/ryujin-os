...
import { DEFAULT_BRAND } from './brandDefaults.js';
...
// Accent color is interpolated UNESCAPED into <style> blocks and inline
// style="" attributes throughout the rendered HTML. An attacker-controlled
// value (e.g. "red}</style><script>...") would break out and run script in
// the same-origin document, stealing the localStorage ryujin_token. Only ever
// emit a strict hex color; anything else falls back to the brand default.
const DEFAULT_ACCENT = '#FF6B00';
function safeAccent(value) {
  return (typeof value === 'string' && /^#[0-9a-fA-F]{3,8}$/.test(value)) ? value : DEFAULT_ACCENT;
}
...
export function renderProposalHTML(proposal) {
  const p = proposal;
  const accent = safeAccent(p.company?.accentColor);
  const companyName = p.company?.name || DEFAULT_BRAND.companyName;
  const customerFirst = (p.customer?.name || 'Homeowner').split(' ')[0];
  const slug = p.proposal?.slug || '';
  const rep = p.salesRep || DEFAULT_BRAND.repProposal;
  ...
  const contactPhone = rep.phone || p.company?.phone || DEFAULT_BRAND.phone;
  ...
  let html = `<!DOCTYPE html><html lang="en"><head>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .section{max-width:800px;margin:0 auto;padding:100px 24px}
    .section-sm{max-width:800px;margin:0 auto;padding:60px 24px}
    .slide-divider{width:60px;height:2px;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.12),transparent);margin:20px auto}
    ...
    /* Hero */
    .prop-hero{padding:140px 24px 100px;text-align:center;min-height:80vh;display:flex;flex-direction:column;align-items:center;justify-content:center}
    ...
    .prop-hero h1{font-size:3.5em;font-weight:900;line-height:1.08;margin-bottom:16px;color:#fff;max-width:700px}
    ...
    /* Price hero */
    .price-slide{text-align:center;padding:100px 24px;min-height:60vh;display:flex;flex-direction:column;align-items:center;justify-content:center}
    ...
    .price-total{font-family:'Montserrat',sans-serif;font-size:5em;font-weight:900;color:#fff;line-height:1;text-shadow:0 0 80px rgba(255,255,255,0.15)}
    ...
    /* Warranty */
    .warranty-slide{text-align:center;padding:100px 24px;min-height:50vh;display:flex;flex-direction:column;align-items:center;justify-content:center}
    .warranty-num{font-family:'Montserrat',sans-serif;font-size:7em;font-weight:900;color:#4ade80;line-height:1;text-shadow:0 0 60px rgba(74,222,128,0.2)}
    ...
    .finance-monthly{font-family:'Montserrat',sans-serif;font-size:4em;font-weight:900;color:#fff;text-shadow:0 0 40px rgba(255,255,255,0.1)}
  ...
  const tierDetails = {
    ...
    'diamond': { shingle: 'CertainTeed Presidential', grade: 'Luxury / Designer', underlayment: 'Premium System', iceShield: 'Grace — Full Coverage', valleys: 'Metal V-Cut', warranty: '25 years', position: 'The finest residential roofing available. 6 bundles per square for a thick, luxury appearance that lasts a lifetime.' }
  };
  ...
  if (showTiers) {
    ...
    const upgradePaths = [
      ...
      { from: 'Platinum', to: 'Diamond', gains: ['CertainTeed Presidential luxury shingles', '6 bundles/SQ (thicker coverage)', 'Full-coverage ice shield', '+5 years warranty'] }
    ];
  ...
  html += `<div class="footer">
    ...
    CertainTeed Certified &middot; Licensed in NB &middot; Locally Owned<br>
...
export function renderContractHTML(contract) {
  const c = contract;
  const accent = safeAccent(c.contractor?.accentColor);
...
export function renderSalesPageHTML(salesData) {
  // ═══════════════════════════════════════════════════════════════
  // PREMIUM SALES PRESENTATION v2
  // Built on Jewels Grace buyer psychology + Hormozi value equation
  // Structure: Hero → Why Now → Video → Trust → How It Works →
  //            What's Included → Gallery → Testimonials → About → CTA → Pricing
  //
  // Psychology: Value before price. Sell the vacation, not the process.
  // Three trust questions: Why us? Why you? Why now?
  // CTB (Call to Behavior) not CTA.
  // ═══════════════════════════════════════════════════════════════
  const s = salesData;
  const brand = s.branding || {};
  const accent = safeAccent(brand.accentColor);
  const companyName = brand.companyName || DEFAULT_BRAND.companyName;
  const customerFirst = (s.hero?.headline || 'Homeowner').split(',')[0].split(' ')[0];
  const customerName = s.hero?.headline?.split(',')[0] || 'Homeowner';
  // Default cover photo — first brand photo (aerial drone shot)
  const DEFAULT_COVER = 'https://assets.cdn.filesafe.space/aHotOUdq9D8m3JPrRz9n/media/683df2a1e1360e210d25da2f.jpeg';
  const coverPhoto = s.coverPhoto || (s.hero?.photos && s.hero.photos.length > 0 ? s.hero.photos[0] : '') || DEFAULT_COVER;
  const packageName = s.package?.name || 'Custom Package';
  const rep = s.salesRep || DEFAULT_BRAND.repSales;
  const tmpl = s.template || {};
  const contactPhone = rep.phone || brand.phone || DEFAULT_BRAND.phone;
  const contactEmail = rep.email || brand.email || DEFAULT_BRAND.email;
  const introVideo = s.introVideo || 'https://oyhn4tqzifmqqj0o.public.blob.vercel-storage.com/darcy-roof%20proposal%20introduction.mp4';
  ...
  // Real testimonials
  const TESTIMONIALS = [
    { text: "Professional crew, clean job site, excellent communication throughout. The whole process was smooth from the first estimate to the final walkthrough.", author: 'Steve V.', rating: 5 },
    { text: "Best roofing experience we've had. Fair pricing, quality materials, and they actually explain what they're doing and why.", author: 'Brad W.', rating: 5 },
    { text: "They showed up when they said they would, finished on time, and the roof looks incredible. No shortcuts, no surprises.", author: 'Tarah M.', rating: 5 }
  ];

  const scopeItems = s.scope || [];
  const isExterior = (s.package?.system || '').includes('exterior') || (s.package?.system || '').includes('combined');
  ...
  // What every package includes — sell the vacation, not the process
  const standardIncludes = [
    { icon: '🛡️', label: 'Complete Tear-Off & Inspection', desc: 'Old materials removed to the deck. We inspect every inch before rebuilding.' },
    { icon: '❄️', label: 'Ice & Water Protection', desc: 'Grace ice shield at eaves, valleys, and penetrations. Critical for Maritime winters.' },
    { icon: '🏗️', label: 'Premium Underlayment', desc: 'Full-deck synthetic moisture barrier. Your last line of defense under the shingles.' },
    { icon: '💨', label: 'Proper Ventilation', desc: 'Ridge vent system for balanced attic airflow. Prevents ice dams and premature aging.' },
    { icon: '🔧', label: 'All New Flashing', desc: 'Drip edge, valleys, pipe boots, step flashing. Every transition sealed tight.' },
    { icon: '🧹', label: 'Full Cleanup & Nail Sweep', desc: 'Magnetic sweep, debris hauled, yard restored. You won\'t know we were there.' },
    { icon: '👁️', label: 'Final Walkthrough Together', desc: 'We walk the job with you before we leave. Your satisfaction confirmed on site.' },
    { icon: '📋', label: 'Workmanship Warranty', desc: 'Your investment protected. 80% of roof failures are installation errors, not materials.' }
  ];

  // Exterior-specific includes
  const exteriorIncludes = [
    { icon: '🧱', label: 'Full Wall Substrate', desc: 'OSB sheathing, housewrap, EPS foam insulation, VentiGrid drainage mat. Structural, not cosmetic.' },
    { icon: '🏠', label: 'Premium Siding Installation', desc: 'Your chosen siding system professionally installed with all trim and accessories.' },
    { icon: '💧', label: 'Soffit, Fascia & Gutters', desc: 'Complete perimeter finishing. Proper drainage and ventilation at every edge.' },
    { icon: '💰', label: 'Remediation Allowance', desc: 'Built-in budget for hidden damage. If we don\'t use it, you get the difference back.' }
  ];

  const includes = isExterior ? [...standardIncludes.slice(0,2), ...exteriorIncludes, ...standardIncludes.slice(-2)] : standardIncludes;

  // ── Build the HTML ──
  let html = `<!DOCTYPE html><html lang="en"><head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="robots" content="noindex, nofollow">
    <title>${esc(customerName)}'s Proposal — ${esc(companyName)}</title>
    <meta property="og:title" content="${esc(customerFirst)}, your proposal is ready.">
    <meta property="og:description" content="${esc(packageName)} package — prepared by ${esc(rep.name)} at ${esc(companyName)}">
    <meta name="theme-color" content="#111111">
  ...
  <div class="hero">
    ${coverPhoto
      ? `<img class="hero-img" src="${esc(coverPhoto)}" alt="Your property" onerror="this.parentElement.querySelector('.hero-img-fallback').style.display='block';this.style.display='none'">
         <div class="hero-img hero-img-fallback" style="display:none;background:linear-gradient(135deg,#1a2a3a 0%,#0a1628 50%,#162032 100%)"></div>`
      : `<div class="hero-img" style="background:linear-gradient(135deg,#1a2a3a 0%,#0a1628 50%,#162032 100%)"></div>`
    }
    <div class="hero-overlay"></div>
    <div class="hero-text">
      <h1>${esc(customerFirst)}, your proposal is ready.</h1>
      <div class="tagline">${esc(packageName)} Package — Prepared by ${esc(rep.name)}</div>
    </div>
    <div class="scroll-hint"><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg></div>
  </div>

  <!-- ═══ SLIDE 2: PERSONAL INTRO — Build the bridge of trust ═══ -->
  <div class="section intro">
    <h2>Hey ${esc(customerFirst)},</h2>
    <p>${esc(tmpl.introMessage || "We took a close look at your property and put together a plan that fits your home, your budget, and your timeline. No pressure, no games. Just honest options from a local crew that takes pride in every job.")}</p>
    <p>We'll walk you through exactly what we're recommending, why, and what it'll cost. Everything's transparent. If you have questions at any point, just call or text.</p>
    <p class="name">— ${esc(rep.name)}, ${esc(rep.title || 'Owner')}</p>
  </div>

  <!-- ═══ SLIDE 3: VIDEO INTRO ═══ -->
  <div class="video-wrap">
    <div class="video-card">
      ${introVideo ? `<video autoplay muted playsinline loop style="width:100%;display:block;border-radius:16px;cursor:pointer" onclick="this.muted=!this.muted;this.nextElementSibling.textContent=this.muted?'🔇 Tap to unmute':'🔊 Playing'">
        <source src="${esc(introVideo)}" type="video/mp4">
      </video>
      <div style="text-align:center;padding:8px;font-size:0.78em;color:rgba(255,255,255,0.35)">🔇 Tap to unmute</div>` : `<div class="video-placeholder">
        <div style="font-size:3em;opacity:0.15">▶</div>
        <div style="font-size:0.9em;color:rgba(255,255,255,0.4)">Personal video intro from ${esc(rep.name.split(' ')[0])}</div>
        <div style="font-size:0.75em;color:rgba(255,255,255,0.2)">We'll record one for your specific project</div>
      </div>`}
    </div>
  </div>

  <!-- ═══ SLIDE 4: WHY US — Three trust questions ═══ -->
  <div class="section" style="text-align:center">
    <h2 style="font-family:'Montserrat',sans-serif;font-size:1.6em;font-weight:800;margin-bottom:40px;color:#fff">Why ${esc(companyName)}?</h2>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:24px;max-width:800px;margin:0 auto">
      <div style="padding:30px 20px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:16px">
        <div style="font-size:2.5em;margin-bottom:12px">🏗️</div>
        <h4 style="font-family:'Montserrat',sans-serif;font-size:0.95em;font-weight:700;color:#fff;margin-bottom:8px">CertainTeed Certified</h4>
        <p style="font-size:0.85em;color:rgba(255,255,255,0.5);line-height:1.6">Factory-trained installation. Premium materials installed the way they're designed to perform.</p>
      </div>
      <div style="padding:30px 20px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:16px">
        <div style="font-size:2.5em;margin-bottom:12px">📋</div>
        <h4 style="font-family:'Montserrat',sans-serif;font-size:0.95em;font-weight:700;color:#fff;margin-bottom:8px">80% of Failures Are Installation</h4>
        <p style="font-size:0.85em;color:rgba(255,255,255,0.5);line-height:1.6">Not materials. Our workmanship warranty protects what actually matters. We stand behind our work.</p>
      </div>
      <div style="padding:30px 20px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:16px">
        <div style="font-size:2.5em;margin-bottom:12px">🏡</div>
        <h4 style="font-family:'Montserrat',sans-serif;font-size:0.95em;font-weight:700;color:#fff;margin-bottom:8px">Your Neighbors Trust Us</h4>
        <p style="font-size:0.85em;color:rgba(255,255,255,0.5);line-height:1.6">35+ five-star reviews. We're the crew your neighbors are choosing. Local, accountable, here to stay.</p>
      </div>
    </div>
  </div>
  ...
  <div class="what-section">
    <div class="section">
      <h2 style="font-family:'Montserrat',sans-serif;font-size:1.4em;font-weight:700;text-align:center;margin-bottom:8px;color:#fff">How It Works</h2>
      <p style="text-align:center;color:rgba(255,255,255,0.45);font-size:0.95em;margin-bottom:30px">Four steps. No surprises.</p>
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;max-width:800px;margin:0 auto">
        <div style="text-align:center;padding:20px 12px">
          <div style="width:48px;height:48px;border-radius:50%;background:${accent};color:#fff;font-family:'Montserrat',sans-serif;font-weight:900;font-size:1.2em;display:flex;align-items:center;justify-content:center;margin:0 auto 12px">1</div>
          <h4 style="font-size:0.85em;font-weight:700;color:#fff;margin-bottom:6px">Inspection</h4>
          <p style="font-size:0.78em;color:rgba(255,255,255,0.45);line-height:1.5">We assess your property and identify exactly what needs to happen.</p>
        </div>
        <div style="text-align:center;padding:20px 12px">
          <div style="width:48px;height:48px;border-radius:50%;background:${accent};color:#fff;font-family:'Montserrat',sans-serif;font-weight:900;font-size:1.2em;display:flex;align-items:center;justify-content:center;margin:0 auto 12px">2</div>
          <h4 style="font-size:0.85em;font-weight:700;color:#fff;margin-bottom:6px">Proposal</h4>
          <p style="font-size:0.78em;color:rgba(255,255,255,0.45);line-height:1.5">You're reading it. Clear pricing, scope, and options. No hidden fees.</p>
        </div>
        <div style="text-align:center;padding:20px 12px">
          <div style="width:48px;height:48px;border-radius:50%;background:${accent};color:#fff;font-family:'Montserrat',sans-serif;font-weight:900;font-size:1.2em;display:flex;align-items:center;justify-content:center;margin:0 auto 12px">3</div>
          <h4 style="font-size:0.85em;font-weight:700;color:#fff;margin-bottom:6px">Installation</h4>
          <p style="font-size:0.78em;color:rgba(255,255,255,0.45);line-height:1.5">Our crew shows up on time, works clean, and keeps you in the loop daily.</p>
        </div>
        <div style="text-align:center;padding:20px 12px">
          <div style="width:48px;height:48px;border-radius:50%;background:${accent};color:#fff;font-family:'Montserrat',sans-serif;font-weight:900;font-size:1.2em;display:flex;align-items:center;justify-content:center;margin:0 auto 12px">4</div>
          <h4 style="font-size:0.85em;font-weight:700;color:#fff;margin-bottom:6px">Walkthrough</h4>
          <p style="font-size:0.78em;color:rgba(255,255,255,0.45);line-height:1.5">We walk it together before we leave. Your satisfaction confirmed on site.</p>
        </div>
      </div>
    </div>
  </div>

  <!-- ═══ SLIDE 6: WHAT'S INCLUDED — Value stack ═══ -->
  <div class="section">
    <h2 style="font-family:'Montserrat',sans-serif;font-size:1.4em;font-weight:700;text-align:center;margin-bottom:8px;color:#fff">What's Included in Your ${esc(packageName)} Package</h2>
    <p style="text-align:center;color:rgba(255,255,255,0.45);font-size:0.95em;margin-bottom:30px">Everything you need. Nothing you don't.</p>
    <div class="what-grid">
      ${includes.map(item => `<div class="what-item"><div class="icon">${item.icon}</div><h4>${esc(item.label)}</h4><p>${esc(item.desc)}</p></div>`).join('')}
    </div>
  </div>

  <!-- ═══ SLIDE 7: OUR WORK — Real project photos ═══ -->
  <div class="section-sm">
  ...
  <!-- ═══ SLIDE 8: TESTIMONIALS — Real reviews ═══ -->
  <div class="section" style="text-align:center">
    <div style="color:#facc15;font-size:1.2em;margin-bottom:8px">⭐⭐⭐⭐⭐</div>
    <h2 style="font-family:'Montserrat',sans-serif;font-size:1.3em;font-weight:700;margin-bottom:4px;color:#fff">What Your Neighbors Are Saying</h2>
    ...
    <div style="max-width:600px;margin:0 auto;display:flex;flex-direction:column;gap:10px">
      ${TESTIMONIALS.map(t => `
        <div style="padding:20px 24px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:14px;text-align:left">
          <div style="color:#facc15;font-size:0.8em;margin-bottom:6px">${'⭐'.repeat(t.rating)}</div>
          <p style="font-size:0.95em;line-height:1.8;color:rgba(255,255,255,0.7);font-style:italic">"${esc(t.text)}"</p>
          <div style="font-size:0.78em;color:rgba(255,255,255,0.35);font-weight:600;margin-top:8px">— ${esc(t.author)}, Google Review</div>
        </div>
      `).join('')}
    </div>
  </div>

  <!-- ═══ SLIDE 9: ABOUT YOUR ESTIMATOR ═══ -->
  <div class="section" style="text-align:center">
    ...
    <div class="about-card">
      <h3>${esc(rep.name)}</h3>
      <div class="title">${esc(rep.title || 'Owner')} &bull; CertainTeed Certified</div>
      <p>${esc(rep.bio || "Whether it's a straightforward reroof or a full Performance Shell renovation, " + rep.name.split(' ')[0] + " is on site for every estimate, every walkthrough, and every question. Local, accountable, and invested in getting it right.")}</p>
  ...
  <div class="cta-section">
    <h2>${esc(customerFirst)}, Ready to Move Forward?</h2>
    <div class="sub">Your detailed proposal with pricing is in the next tab. When you're ready, let's talk.</div>
    <a href="tel:${esc((rep.phoneTel||contactPhone).replace(/[^0-9]/g,''))}" class="cta-btn">Call ${esc(rep.name.split(' ')[0])} — ${esc(contactPhone)}</a>
    <a href="mailto:${esc(contactEmail)}?subject=Re:%20${encodeURIComponent(packageName)}%20Proposal" class="cta-alt">Or email ${esc(rep.name.split(' ')[0])}</a>
    <div class="financing">Financing available through FinanceIt &bull; As low as $99/mo</div>
  </div>

  <div style="display:none">
  ${(() => {
    // Get pricing from either proposal data or sales page pricing
    const p = s._proposalData;
    const sp = s.pricing || {};
    const hasProposalItems = p && p.lineItems && p.lineItems.length > 0;
    const totalStr = hasProposalItems ? (p.pricing?.total || '') : (sp.total || sp.totalFormatted || '');
    const subtotalStr = hasProposalItems ? (p.pricing?.subtotal || '') : (sp.subtotal || '');
    const taxStr = hasProposalItems ? (p.pricing?.tax || '') : (sp.tax || '');
    const taxLabel = hasProposalItems ? (p.pricing?.taxLabel || 'HST') : (sp.taxLabel || 'HST');
    const warrantyStr = hasProposalItems ? (p.warranty || '') : '';

    if (!totalStr) return ''; // no pricing data at all
    return `
    <div style="max-width:800px;margin:0 auto;padding:60px 24px 40px">
      <div style="text-align:center;margin-bottom:40px">
        <div style="font-size:0.72em;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:rgba(255,255,255,0.35);margin-bottom:12px">Your Investment</div>
        <h2 style="font-family:'Montserrat',sans-serif;font-size:2.4em;font-weight:900;color:#fff;margin-bottom:8px">${esc(totalStr)}</h2>
        <div style="font-size:1em;color:rgba(255,255,255,0.5)">${esc(packageName)} Package &bull; ${esc(taxLabel)} included</div>
      </div>

      <div style="max-width:500px;margin:0 auto">
        ${hasProposalItems ? p.lineItems.map(li => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
            <div>
              <div style="font-weight:600;color:#fff;font-size:0.95em">${esc(li.label)}</div>
              ${li.note ? `<div style="font-size:0.78em;color:rgba(255,255,255,0.35);margin-top:2px">${esc(li.note)}</div>` : ''}
            </div>
            <div style="font-family:'Montserrat',sans-serif;font-weight:700;color:#fff;font-size:1em">${esc(li.price)}</div>
          </div>
        `).join('') : (sp.lineItems || []).map(li => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
            <div style="font-weight:600;color:#fff;font-size:0.95em">${esc(li.label)}</div>
            <div style="font-family:'Montserrat',sans-serif;font-weight:700;color:#fff;font-size:1em">${esc(li.price)}</div>
          </div>
        ...
        ${subtotalStr ? `
        <div style="display:flex;justify-content:space-between;padding:16px 0 8px;margin-top:8px;font-size:0.95em">
          <span style="color:rgba(255,255,255,0.5)">Subtotal</span>
          <span style="color:#fff;font-weight:600">${esc(subtotalStr)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:8px 0;font-size:0.95em">
          <span style="color:rgba(255,255,255,0.5)">${esc(taxLabel)}</span>
          <span style="color:#fff;font-weight:600">${esc(taxStr)}</span>
        </div>` : ''}
        <div style="display:flex;justify-content:space-between;padding:20px 0;border-top:2px solid rgba(255,255,255,0.15);margin-top:8px;font-size:1.1em">
          <span style="color:#fff;font-weight:800">Total</span>
          <span style="color:#fff;font-weight:900;font-family:'Montserrat',sans-serif;font-size:1.3em">${esc(totalStr)}</span>
        </div>
      </div>

      ${warrantyStr ? `<div style="text-align:center;margin-top:32px;padding:20px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px;font-size:0.88em;color:rgba(255,255,255,0.6)">${esc(warrantyStr)}</div>` : ''}

      <div style="text-align:center;margin-top:40px">
        <a href="tel:${esc((rep.phoneTel||contactPhone).replace(/[^0-9]/g,''))}" style="display:inline-block;padding:16px 48px;background:${esc(accent)};color:#fff;border-radius:10px;font-family:'Montserrat',sans-serif;font-weight:800;font-size:1.05em;text-decoration:none">Let's Do This — Call ${esc(rep.name.split(' ')[0])}</a>
        <div style="margin-top:16px;font-size:0.85em;color:rgba(255,255,255,0.35)">Financing available through FinanceIt &bull; Valid for 30 days</div>
      </div>
    </div>`;
  })()}
  </div>
  ...
  <div class="footer">
    <span class="brand">${esc(companyName.toUpperCase())}</span>
    ${esc(brand.tagline || DEFAULT_BRAND.tagline)} &middot; Riverview, NB<br>
    <a href="tel:${esc(contactPhone.replace(/[^0-9]/g,''))}">${esc(contactPhone)}</a> &middot;
