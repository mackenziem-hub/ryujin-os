...
export function renderProposalHTML(proposal) {
  ...
  <div class="co-bar">
    <div style="display:flex;align-items:center;gap:16px">
      ${p.company?.logoUrl
        ? `<img src="${esc(p.company.logoUrl)}" style="width:56px;height:56px;border-radius:12px;object-fit:contain" alt="Logo">`
        : `<div style="width:56px;height:56px;border-radius:12px;background:${accent};display:flex;align-items:center;justify-content:center;font-weight:900;font-size:1.4em;color:#fff">${companyName.charAt(0)}</div>`
      }
      <div>
...
export function renderSalesPageHTML(salesData) {
  ...
  const customerFirst = (s.hero?.headline || '').includes(',')
    ? s.hero.headline.split(',')[0]
    : 'Homeowner';
  ...
  const SALES_STYLES = `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Hero */
    .hero-section {
      background: linear-gradient(180deg, #1a1a2e 0%, #2d1b4e 60%, #1a1a2e 100%);
  ...
  let html = `<!DOCTYPE html><html lang="en"><head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${esc(s.hero?.headline || 'Your Custom Roofing Plan')} | ${esc(companyName)}</title>
    <style>${SALES_STYLES}</style>
  </head><body>

  <!-- ═══ HERO ═══ -->
  <div class="hero-section">
    ${(s.hero?.photos && s.hero.photos.length > 0)
      ? `<img class="hero-cover" src="${esc(s.hero.photos[0])}" alt="Your property">`
      : `<div class="hero-cover-placeholder"><div style="font-size:3em;opacity:0.2">🏠</div><div style="font-size:0.85em;opacity:0.4">Cover photo added before sending</div></div>`
    }
    <div class="hero-content container">
      <h1>${esc(s.hero?.headline || 'Your Custom Roofing Plan')}</h1>
      <div class="sub">${esc(s.hero?.subheadline || '')}</div>
      <div class="hero-badge">${esc(s.package?.name || 'Custom Package')}</div>
    </div>
  </div>

  <!-- ═══ WARM INTRO — Connect before converting (Jewels) ═══ -->
  <div class="intro-section">
    <div class="container">
      <div class="letter">
        Hi ${esc(customerFirst)},<br><br>
        Thanks for taking the time to chat with us about your home. We took a close look at everything and put together a plan that's specifically designed for your property.<br><br>
        Below you'll see exactly what's included in your package — no surprises, no hidden costs. We want you to feel confident about every detail before making a decision.<br><br>
        Take a look, and when you're ready, we'll walk you through the full proposal with pricing.
      </div>
      <div class="sign">${esc(companyName)}</div>
      <div class="role">Your Local Roofing Team</div>
    </div>
  </div>

  <!-- ═══ VIDEO — Personal intro from salesperson ═══ -->
  <div class="video-section container">
    <div class="video-placeholder">
      <div style="font-size:2.5em;opacity:0.2">▶</div>
      <div style="font-size:0.85em;color:#999">Personal video intro</div>
      <div style="font-size:0.72em;color:#bbb">Recorded on location for your project</div>
    </div>
  </div>

  <!-- ═══ WHAT'S INCLUDED — Value before price ═══ -->
      ...
      <div class="included-grid">
        ${standardIncludes.map(item => `
          <div class="included-item">
            ...
            <div><div class="included-label">${item.label}</div><div class="included-desc">${item.desc}</div></div>
          </div>
        `).join('')}
      </div>
    </div>
  ...
  <!-- ═══ SCOPE SECTIONS — What they're getting ═══ -->
  ${s.scope && s.scope.length > 0 ? `
  <div class="crew-section">
    <div class="container">
      <h2>Your Project Scope</h2>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;max-width:600px;margin:0 auto;text-align:left">
        ${s.scope.map(section => `
          <div style="padding:20px;background:#fafbfc;border-radius:12px">
            <div style="font-weight:700;font-size:0.9em;margin-bottom:8px;color:${accent}">${esc(section.title)}</div>
            <div style="font-size:0.82em;color:#666;line-height:1.6">${esc(section.description || '')}</div>
            ${section.items ? `<ul style="margin-top:8px;padding-left:16px;font-size:0.78em;color:#888">${section.items.map(i => `<li style="padding:2px 0">${esc(i)}</li>`).join('')}</ul>` : ''}
          </div>
        `).join('')}
      </div>
    </div>
  ...
  <!-- ═══ CREW — Social proof through presence ═══ -->
  <div class="crew-section" style="background:#fafbfc">
    <div class="container">
      <h2>Our Crew</h2>
      <div class="crew-grid">
        ${[1,2,3,4,5,6].map(i => `
          <div class="crew-photo">
            <div style="font-size:1.5em;opacity:0.15">👷</div>
          </div>
        `).join('')}
      </div>
      <p style="font-size:0.82em;color:#888;margin-top:16px">Professional, insured & certified. Every member of our team is trained to manufacturer specifications.</p>
    </div>
  </div>

  <!-- ═══ ABOUT YOUR ESTIMATOR ═══ -->
  <div class="about-section">
    <div class="container">
      <h2>About Your Estimator</h2>
      <div class="about-card">
        <div class="about-avatar">M</div>
        <div class="about-info">
          <h3>Mackenzie Mazerolle</h3>
          <div class="title">Owner &bull; CertainTeed Certified</div>
          <p>Third-generation roofer with 20+ years of experience in the industry. Locally owned and operated — we live in the community we serve. Every estimate is personally reviewed to make sure you're getting the right solution for your home.</p>
          <div class="about-contact">
            ${brand.phone ? `<a href="tel:${esc(brand.phone)}">${esc(brand.phone)}</a> &bull; ` : ''}
  ...
  <div style="padding:48px 32px;text-align:center">
    <div class="container">
      <h2 style="font-size:1.3em;font-weight:800;margin-bottom:20px">What Your Neighbors Are Saying</h2>
      ${(s.testimonials || []).map(t => `
        <div style="background:#fafbfc;border-radius:14px;padding:24px 28px;margin-bottom:12px;text-align:left;max-width:600px;margin-left:auto;margin-right:auto;position:relative">
          <div style="font-size:2em;color:#ddd;position:absolute;top:8px;left:16px;font-family:Georgia">&ldquo;</div>
          <div style="padding-left:28px;font-style:italic;color:#555;line-height:1.7;font-size:0.92em">${esc(t.text)}</div>
          <div style="text-align:right;margin-top:8px;font-weight:700;font-size:0.8em;color:#999">— ${esc(t.author || 'Local Homeowner')}${t.rating ? ' ⭐'.repeat(t.rating) : ''}</div>
        </div>
      `).join('')}
    </div>
  </div>

  <!-- ═══ CTA — View your full proposal ═══ -->
  <div class="cta-section">
    <div class="container">
      <h2>Ready to See Your Custom Plan?</h2>
      <p>Your full proposal with pricing, scope details, warranty information, and financing options is ready for you.</p>
      <a href="${esc(s.cta?.acceptUrl || '#')}" class="cta-btn">${esc(s.cta?.label || 'View Your Proposal')}</a>
      <div style="margin-top:16px;font-size:0.78em;opacity:0.5">
        ${s.cta?.validDays ? `This proposal is valid for ${s.cta.validDays} days` : ''}
      </div>
    </div>
  </div>
  ...
  <div class="sales-footer">
    <div class="container">
      ...
      ${brand.phone ? `<a href="tel:${esc(brand.phone)}">${esc(brand.phone)}</a> &bull; ` : ''}
      ${brand.email ? `<a href="mailto:${esc(brand.email)}">${esc(brand.email)}</a> &bull; ` : ''}
      ${brand.website ? `<a href="${esc(brand.website)}">${esc(brand.website)}</a>` : ''}
      <div style="margin-top:8px;font-size:0.9em">CertainTeed Certified &bull; Fully Insured &bull; Locally Owned</div>
    </div>
  </div>
