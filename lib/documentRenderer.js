...
export function renderProposalHTML(proposal) {
  ...
  const contactPhone = p.company?.phone || '(506) 540-1052';
  ...
  <!-- ═══ SLIDE 1: HERO ═══ -->
  <!-- ═══ SLIDE 1: HERO — Personal, warm ═══ -->
  <div class="prop-hero">
    <div class="label">Proposal</div>
    <h1>${esc(customerFirst)}, here's your<br>custom roofing plan.</h1>
    <div class="addr">${esc(p.customer?.address || '')}</div>
  ...
  <div class="section" style="text-align:center;max-width:650px">
    ...
    <p style="font-size:1.05em;line-height:1.9;color:rgba(255,255,255,0.7)">Thanks for having us out. We've put together a custom plan based on what we found during the inspection. Everything below is tailored to your home — the materials, the scope, the pricing.</p>
    <p style="font-size:0.9em;color:rgba(255,255,255,0.4);margin-top:20px;font-style:italic">— ${esc(p.proposal?.preparedBy || 'Mackenzie')}, ${esc(companyName)}</p>
  </div>
  ...
  html += `<div class="cta-section">
    ...
    <a href="tel:${esc(contactPhone.replace(/[^0-9]/g,''))}" class="cta-btn">Call ${esc(contactPhone)}</a>
    <a href="mailto:${esc(p.company?.email || '')}" class="cta-alt">Or email ${esc(p.company?.email || '')}</a>
  </div>`;
...
/* OLD WHITE PROPOSAL CONTENT REMOVED — replaced with dark slide-based presentation above */
function __removed() {
...
export function renderSalesPageHTML(salesData) {
  ...
  const contactPhone = brand.phone || '(506) 540-1052';
  const contactEmail = brand.email || 'plusultraroofing@gmail.com';
  const customMessage = s.hero?.subheadline
    ? `We took a close look at your property and put together a plan that fits your home, your budget, and your timeline. No pressure, no games — just honest options from a local crew that takes pride in every job.`
    : `We took a close look at your property and put together a plan that fits your home, your budget, and your timeline. No pressure, no games — just honest options from a local crew that takes pride in every job.`;
  ...
  <div class="hero">
    ...
    <div class="hero-text">
      <h1>Your System. Your Timeline.<br>Your Decision.</h1>
      <div class="tagline">Same professional standard. Different performance levels.</div>
    </div>
  ...
  <div class="section intro">
    ...
    <p>${customMessage}</p>
    <p class="name">— Mackenzie &amp; the ${esc(companyName)} crew</p>
  </div>
  ...
  <!-- ═══ SLIDE 5.5: TESTIMONIAL ═══ -->
  <div class="testimonial-section">
    <div class="testimonial-text">"Professional crew, clean job site, excellent communication throughout. The whole process was smooth from the first estimate to the final walkthrough. Would definitely recommend to anyone in the area."</div>
    <div class="testimonial-author">— Recent Customer, ${new Date().getFullYear()}</div>
  </div>
  ...
  <div class="section" style="text-align:center">
    ...
    <div class="about-card">
      ...
      <p>Mackenzie is the owner of ${esc(companyName)} — a third-generation roofing company serving Greater Moncton and beyond. He's hands-on, tech-forward, and committed to doing every job right.</p>
      <div class="about-links">
        <a href="tel:${esc(contactPhone.replace(/[^0-9]/g,''))}">${esc(contactPhone)}</a>
  ...
  <div class="cta-section">
    ...
    <a href="tel:${esc(contactPhone.replace(/[^0-9]/g,''))}" class="cta-alt">Or call Mackenzie at ${esc(contactPhone)}</a>
