...
export function generateProposal(quoteResult, options = {}) {
  const {
    ...
    multiOfferResults = null,
    salesRep = null,
    inlineComparison = null,
    template = null,          // { headline, tagline, introMessage, scopeIntro, tierExplanation }
    templateKey = '',
    notes = '',
  ...
  return {
    ...
    // Package comparison (for side-by-side in proposal)
    comparison: multiOfferResults && multiOfferResults.offers
      ? Object.entries(multiOfferResults.offers).map(([slug, q]) => ({
          slug,
          name: q.offer.name,
          badge: q.offer.badge,
...
export function generateSalesPageData(quoteResult, options = {}) {
  ...
  const offer = quoteResult.offer || {};
  const summary = quoteResult.summary || {};
  const lineItems = quoteResult.lineItems || [];

  // Bundled retail for display
  const retailItems = bundleLineItems(lineItems, summary.sellingPrice || 0, summary.hardCost || 0);
  ...
  return {
    ...
    pricing: {
      ...
      subtotal: `$${(summary.sellingPrice||0).toLocaleString()}`,
      tax: `$${(summary.tax||0).toLocaleString()} ${summary.taxLabel || 'HST'}`,
      total: `$${(summary.totalWithTax||0).toLocaleString()}`,
      totalRaw: summary.totalWithTax || 0,
...
function getRoofingDescription(offer) {
  if (!offer) return 'Professional roofing system.';
