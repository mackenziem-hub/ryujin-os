...
// mergeRows / dedupe (ADDRESS-FIRST cross-store merge) live in
// lib/proposalsDedupe.js and are imported above.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const counts = { total: merged.length, pending: 0, byBucket: {}, byStore: { 'Estimator OS': est.rows.length, 'Ryujin-native': native.rows.length, 'GHL': ghl.rows.length, 'manual': manual.rows.length }, noPrice: 0 };
